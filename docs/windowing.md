...
| **4** | Add late-data policy (configurable: drop, allowed lateness, side output). **Done:** `LateDataPolicy` enum (Drop, SideOutput) in `src/nodes/stream/late_data_policy.rs`. All event-time window nodes support `with_late_data_policy(policy)`. Drop (default): discard late events. SideOutput: route late events to `"late"` output port. |
