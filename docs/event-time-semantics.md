...
| Phase | Content |
|-------|--------|
| **1** | Document event time vs processing time; define convention or trait for “event time” on payloads. |
| **2** | Ensure timestamped execution path can use **external** event time (user-provided) instead of only internal counter. |
