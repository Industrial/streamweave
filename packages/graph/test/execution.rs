...
use streamweave_graph::{
  ConsumerNode, ExecutionError, ExecutionState, Graph, GraphBuilder, GraphExecution, GraphExecutor,
  ProducerNode, TransformerNode,
};
...
mod lifecycle_tests {
  ...
  async fn test_pause_resume_lifecycle() {
    ...
    executor.pause().await.unwrap();
    assert_eq!(executor.state(), streamweave_graph::ExecutionState::Paused);
    assert!(!executor.is_running());
...
mod additional_execution_coverage {
  ...
  fn test_execution_state_display() {
    ...
    assert_eq!(display_str, "Stopped");

    let state = ExecutionState::Running;
    let display_str = format!("{}", state);
    assert_eq!(display_str, "Running");
