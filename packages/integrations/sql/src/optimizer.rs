...
impl QueryOptimizer {
  ...
  pub fn optimize(&self, mut query: SqlQuery) -> Result<SqlQuery, Box<StreamError<String>>> {
    ...
    query.select.items = query
      ...
      .iter()
      .map(|item| match item {
        crate::ast::SelectItem::Expression { expr, alias } => self
          .fold_constants(expr.clone())
          .map(|expr| crate::ast::SelectItem::Expression {
            expr,
            alias: alias.clone(),
          })
          .map_err(Box::new),
        _ => Ok::<crate::ast::SelectItem, Box<StreamError<String>>>(item.clone()),
      })
  ...
  fn fold_constants(&self, expr: Expression) -> Result<Expression, StreamError<String>> {
    match expr {
      ...
      Expression::UnaryOp { op, operand } => {
        ...
        if let Expression::Literal(ref lit) = operand_folded {
  ...
  fn evaluate_binary_op(
    ...
    match (left, op, right) {
      (Literal::Integer(l), crate::ast::BinaryOperator::Add, Literal::Integer(r)) => {
        Ok(Some(Literal::Integer(l + r)))
      }
      (Literal::Integer(l), crate::ast::BinaryOperator::Subtract, Literal::Integer(r)) => {
        Ok(Some(Literal::Integer(l - r)))
      }
      (Literal::Integer(l), crate::ast::BinaryOperator::Multiply, Literal::Integer(r)) => {
        Ok(Some(Literal::Integer(l * r)))
      }
      (Literal::Integer(l), crate::ast::BinaryOperator::Divide, Literal::Integer(r)) => {
        if *r == 0 {
          return Err(self.create_error("Division by zero".to_string()));
        }
        Ok(Some(Literal::Integer(l / r)))
      }
      (Literal::Integer(l), crate::ast::BinaryOperator::Eq, Literal::Integer(r)) => {
        Ok(Some(Literal::Boolean(l == r)))
      }
      (Literal::Integer(l), crate::ast::BinaryOperator::Ne, Literal::Integer(r)) => {
        Ok(Some(Literal::Boolean(l != r)))
      }
      (Literal::Integer(l), crate::ast::BinaryOperator::Lt, Literal::Integer(r)) => {
        Ok(Some(Literal::Boolean(l < r)))
      }
      (Literal::Integer(l), crate::ast::BinaryOperator::Le, Literal::Integer(r)) => {
        Ok(Some(Literal::Boolean(l <= r)))
      }
      (Literal::Integer(l), crate::ast::BinaryOperator::Gt, Literal::Integer(r)) => {
        Ok(Some(Literal::Boolean(l > r)))
      }
      (Literal::Integer(l), crate::ast::BinaryOperator::Ge, Literal::Integer(r)) => {
        Ok(Some(Literal::Boolean(l >= r)))
      }
      (Literal::Float(l), crate::ast::BinaryOperator::Add, Literal::Float(r)) => {
        Ok(Some(Literal::Float(l + r)))
      }
      (Literal::Float(l), crate::ast::BinaryOperator::Subtract, Literal::Float(r)) => {
        Ok(Some(Literal::Float(l - r)))
      }
      (Literal::Float(l), crate::ast::BinaryOperator::Multiply, Literal::Float(r)) => {
        Ok(Some(Literal::Float(l * r)))
      }
      (Literal::Float(l), crate::ast::BinaryOperator::Divide, Literal::Float(r)) => {
        if *r == 0.0 {
          return Err(self.create_error("Division by zero".to_string()));
        }
        Ok(Some(Literal::Float(l / r)))
      }
      (Literal::Boolean(l), crate::ast::BinaryOperator::And, Literal::Boolean(r)) => {
        Ok(Some(Literal::Boolean(*l && *r)))
      }
      (Literal::Boolean(l), crate::ast::BinaryOperator::Or, Literal::Boolean(r)) => {
        Ok(Some(Literal::Boolean(*l || *r)))
      }
  ...
  fn evaluate_unary_op(
    ...
    match (operand, op) {
      (Literal::Integer(n), crate::ast::UnaryOperator::Minus) => Ok(Some(Literal::Integer(-n))),
      (Literal::Float(n), crate::ast::UnaryOperator::Minus) => Ok(Some(Literal::Float(-n))),
      (Literal::Boolean(b), crate::ast::UnaryOperator::Not) => Ok(Some(Literal::Boolean(!b))),
      _ => Ok(None),
    }
  }
...
mod tests {
  ...
  fn test_constant_folding_comparison() {
    ...
    let expr = Expression::BinaryOp {
      ...
      op: crate::ast::BinaryOperator::Gt,
