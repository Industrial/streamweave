//! Comprehensive tests for SqliteConsumer
...
async fn test_sqlite_consumer_new() {
  ...
  let consumer = SqliteConsumer::new(config.clone());
...
async fn test_sqlite_consumer_with_error_strategy() {
  ...
  let consumer = SqliteConsumer::new(config).with_error_strategy(ErrorStrategy::Skip);
...
async fn test_sqlite_consumer_with_name() {
  ...
  let consumer = SqliteConsumer::new(config).with_name("test_consumer".to_string());
...
async fn test_sqlite_consumer_clone() {
  ...
  let consumer = SqliteConsumer::new(config).with_name("test".to_string());
...
async fn test_sqlite_consumer_consume_single_row() {
  ...
  let mut consumer = SqliteConsumer::new(config);
...
async fn test_sqlite_consumer_component_info() {
  ...
  assert_eq!(
    info.type_name,
    "streamweave_database_sqlite::sqlite_consumer::SqliteConsumer"
  );
}
