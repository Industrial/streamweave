...
use super::moving_average_transformer::{MovingAverageState, MovingAverageTransformer};
...
impl StateStore<MovingAverageState> for SharedMovingAverageStore {
  fn get(&self) -> streamweave_stateful::StateResult<Option<MovingAverageState>> {
    self.0.get()
  }

  fn set(&self, state: MovingAverageState) -> streamweave_stateful::StateResult<()> {
    self.0.set(state)
  }
  ...
  ) -> streamweave_stateful::StateResult<MovingAverageState> {
