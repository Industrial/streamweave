...
///
/// ```rust
/// use streamweave_transformers::{MessageDedupeTransformer, DeduplicationWindow};
...
where
  T: std::fmt::Debug + Clone + Send + Sync + 'static,
{
  type InputPorts = (streamweave_message::Message<T>,);
  type OutputPorts = (streamweave_message::Message<T>,);
