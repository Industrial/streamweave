...
//!
//! ```rust
//! use streamweave::graph::batching::{BatchBuffer, BatchConfig};
//! use bytes::Bytes;
//!
...
use super::channels::ChannelItem;
use super::execution::BatchConfig;
use bytes::Bytes;
use std::sync::Arc;
...
pub struct BatchingChannel {
  ...
  inner: super::channels::TypeErasedSender,
...
impl BatchingChannel {
  ...
  pub fn new(sender: super::channels::TypeErasedSender, config: BatchConfig) -> Self {
  ...
  pub async fn send(
    &self,
    item: super::channels::ChannelItem,
  ) -> Result<(), super::execution::ExecutionError> {
    // Extract Bytes from ChannelItem
    let bytes = match item {
      super::channels::ChannelItem::Bytes(b) => b,
      _ => {
        // For non-Bytes items, send directly without batching
        return self.inner.send(item).await.map_err(|e| {
          super::execution::ExecutionError::ChannelError {
    ...
    let should_flush = {
      ...
      buffer.add(bytes).map_err(|e| {
        super::execution::ExecutionError::ExecutionFailed(format!(
  ...
  pub async fn flush(&self) -> Result<(), super::execution::ExecutionError> {
    let items = {
      let mut buffer = self.buffer.lock().await;
      buffer.flush()
    };
    ...
    // Serialize batch
    let batch_bytes =
      serialize_batch(items).map_err(|e| super::execution::ExecutionError::SerializationError {
        node: "unknown".to_string(),
    ...
    self
      .inner
      .send(super::channels::ChannelItem::Bytes(batch_bytes))
      .await
      .map_err(|e| super::execution::ExecutionError::ChannelError {
        node: "unknown".to_string(),
        port: "unknown".to_string(),
        is_input: false,
  ...
  pub async fn shutdown(&self) -> Result<(), super::execution::ExecutionError> {
  ...
  pub fn inner(&self) -> &super::channels::TypeErasedSender {
    &self.inner
  }
}
