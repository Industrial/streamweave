...
//! blanket implementation, enabling zero-copy sharing using `Arc<Message<T>>`.
//! This is used extensively in in-process execution mode for fan-out scenarios.
//!
...
/// ```rust
/// use std::sync::Arc;
/// use streamweave::graph::ZeroCopyShare;
///
/// let data = vec![1, 2, 3];
...
/// in fan-out scenarios. This is particularly useful for broadcast routers where
/// the same `Message<T>` needs to be sent to multiple consumers.
///
/// # Example
///
/// ```rust
...
/// use std::sync::Arc;
///
/// let pool = ArcPool::<streamweave::message::Message<i32>>::new(10);
/// let msg = wrap_message(42i32);
/// let arc = pool.get_or_create(msg);
/// // Use arc (Arc<Message<i32>>) for zero-copy fan-out...
/// pool.return_arc(arc); // Return to pool for reuse
/// ```
...
///
/// ```rust
/// use streamweave::graph::InPlaceTransform;
///
/// struct VecDoubler;
///
...
///
/// ```rust
/// use streamweave::graph::MutateInPlace;
///
/// let mut vec = vec![1, 2, 3];
