...
//! before transmission. This ensures:
//!
//! - **Message IDs**: Every item has a unique identifier for tracking
//! - **Metadata Preservation**: Message metadata is preserved through transformations
...
//!
//! Nodes handle `Message<T>` wrapping and unwrapping automatically:
...
//!
//! - **Port Bounds**: Port names are validated at compile time through the
//!   `HasOutputPort` and `HasInputPort` traits, which ensure ports exist.
//!   Attempting to use an invalid port name will fail to compile.
//!
...
//!
//! // Create a graph - data flows as Message<T> internally
...
use super::connection::{CompatibleWith, HasInputPort, HasOutputPort};
...
pub struct Graph {
  ...
  execution_mode: super::execution::ExecutionMode,
}
...
impl Graph {
  ...
  pub fn new() -> Self {
    Self {
      nodes: HashMap::new(),
      connections: Vec::new(),
      execution_mode: super::execution::ExecutionMode::new_in_process(), // Default to in-process for zero-copy
    }
  }
  ...
  /// ```
  #[must_use]
  pub fn with_execution_mode(mut self, mode: super::execution::ExecutionMode) -> Self {
  ...
  #[must_use]
  pub fn execution_mode(&self) -> &super::execution::ExecutionMode {
    &self.execution_mode
  }
...
pub struct GraphBuilder<State = Empty> {
  ...
  connections: Vec<ConnectionInfo>,
  _state: State,
  execution_mode: Option<super::execution::ExecutionMode>,
}
...
impl GraphBuilder<Empty> {
  ...
  ///
  /// **Note:** All data flowing through graphs is automatically wrapped in `Message<T>`.
  /// Port types use raw types (e.g., `i32`), but nodes handle `Message<T>` wrapping
...
impl<Nodes, Connections> GraphBuilder<HasConnections<Nodes, Connections>> {
  ...
  ///
  /// The returned graph is ready for execution. All data flowing through the graph
  /// will be automatically wrapped in `Message<T>`, with message IDs and metadata
  /// preserved through transformations.
  ///
  /// # Returns
  ///
  /// A `Graph` instance containing all added nodes and connections, ready for execution.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  ...
  ///     .build();
  ///
  /// // Execute the graph - data flows as Message<T>
  ...
  pub fn build(self) -> Graph {
    Graph {
      ...
      execution_mode: self
        .execution_mode
        .unwrap_or_else(super::execution::ExecutionMode::new_in_process),
    }
  }
}
