...
impl Output for NumberOutput {
  type Output = i32;
  type OutputStream = Pin<Box<dyn futures::Stream<Item = i32> + Send>>;
}
...
impl Output for TextOutput {
  type Output = String;
  type OutputStream = Pin<Box<dyn futures::Stream<Item = String> + Send>>;
}
...
async fn test_number_output_stream() {
  ...
  let stream: Pin<Box<dyn futures::Stream<Item = i32> + Send>> =
    Box::pin(stream::iter(output.data));
...
async fn test_output_stream_composition() {
  ...
  let stream1: Pin<Box<dyn futures::Stream<Item = i32> + Send>> =
    Box::pin(stream::iter(output1.data));
  let stream2: Pin<Box<dyn futures::Stream<Item = i32> + Send>> =
    Box::pin(stream::iter(output2.data));
