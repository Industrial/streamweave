  ...
  let handle = node
    .spawn_execution_task(
      ...
      ExecutionMode::Distributed {
        serializer: streamweave::graph::serialization::JsonSerializer,
        compression: None,
        batching: None,
      },
      None,
  ...
  while let Some(channel_item) = rx.recv().await {
    match channel_item {
      ChannelItem::Bytes(bytes) => {
        let msg: Message<i32> = deserialize(bytes).unwrap();
...
async fn test_producer_execution_empty_stream() {
  use streamweave::graph::channels::{TypeErasedReceiver, TypeErasedSender};
...
async fn test_producer_execution_different_types() {
  ...
  while let Some(channel_item) = rx.recv().await {
    match channel_item {
      ChannelItem::Bytes(bytes) => {
        let msg: Message<String> = deserialize(bytes).unwrap();
        received.push(msg.into_payload());
      }
      _ => panic!("Unexpected channel item variant"),
    }
  }
...
async fn test_producer_execution_custom_struct() {
  ...
  while let Some(channel_item) = rx.recv().await {
    match channel_item {
      ChannelItem::Bytes(bytes) => {
        let msg: Message<TestStruct> = deserialize(bytes).unwrap();
        received.push(msg.into_payload());
      }
      _ => panic!("Unexpected channel item variant"),
    }
...
async fn test_producer_in_process_zero_copy() {
  ...
  while let Some(channel_item) = rx.recv().await {
    match channel_item {
      ChannelItem::Arc(arc) => {
        // Downcast to Arc<Message<i32>>
        let msg_arc = Arc::downcast::<Message<i32>>(arc)
          .map_err(|_| "Failed to downcast to Message<i32>")
          .unwrap();
        let msg = Arc::try_unwrap(msg_arc).unwrap_or_else(|arc| (*arc).clone());
        received.push(*msg.payload());
      }
      ChannelItem::Bytes(_) => {
...
async fn test_producer_in_process_fan_out_zero_copy() {
  ...
  match (item1, item2) {
    (ChannelItem::Arc(arc1), ChannelItem::Arc(arc2)) => {
      // Both should be Arc<Message<i32>>
      let msg1_arc = arc1
        .downcast::<Message<i32>>()
        .map_err(|_| "Failed to downcast to Message<i32>")
        .unwrap();
      let msg2_arc = arc2
        .downcast::<Message<i32>>()
        .map_err(|_| "Failed to downcast to Message<i32>")
        .unwrap();
      let msg1 = Arc::try_unwrap(msg1_arc).unwrap_or_else(|arc| (*arc).clone());
      let msg2 = Arc::try_unwrap(msg2_arc).unwrap_or_else(|arc| (*arc).clone());

      // Verify values are the same
      assert_eq!(*msg1.payload(), 42);
      assert_eq!(*msg2.payload(), 42);

      // Verify they are the same Arc (same memory location) - messages should share the same Arc
      // In fan-out, we use Arc::clone which shares the same underlying data
      // Note: We can't directly compare Arc pointers after unwrapping, but we can verify the payloads match
    }
...
async fn test_transformer_in_process_zero_copy() {
  ...
  // Send input as Arc<Message<i32>>
  use streamweave::message::wrap_message;
  let input_msg = wrap_message(21);
  input_tx
    .send(ChannelItem::Arc(
      Arc::new(input_msg) as Arc<dyn std::any::Any + Send + Sync>
    ))
    .await
    .unwrap();
  drop(input_tx); // Close input channel

  // Receive output - should be Arc<Message<i32>> in in-process mode
...
async fn test_consumer_in_process_zero_copy() {
  ...
  let msg2 = wrap_message(100);
  input_tx
    .send(ChannelItem::Arc(
      Arc::new(msg1) as Arc<dyn std::any::Any + Send + Sync>
    ))
    .await
    .unwrap();
  input_tx
    .send(ChannelItem::Arc(
      Arc::new(msg2) as Arc<dyn std::any::Any + Send + Sync>
    ))
    .await
    .unwrap();
  drop(input_tx); // Close input channel
