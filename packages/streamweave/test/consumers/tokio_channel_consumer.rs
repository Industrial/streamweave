...
#[tokio::test]
async fn test_tokio_channel_consumer_basic() {
...
#[tokio::test]
async fn test_tokio_channel_consumer_empty() {
  let (tx, mut rx) = mpsc::channel(10);
  let mut consumer = TokioChannelConsumer::new(tx);
...
#[tokio::test]
async fn test_tokio_channel_consumer_with_name() {
  let (tx, _rx) = mpsc::channel(10);
  let consumer = TokioChannelConsumer::new(tx).with_name("channel_consumer".to_string());

  assert_eq!(consumer.config.name, "channel_consumer");
}

#[tokio::test]
async fn test_tokio_channel_consumer_with_error_strategy() {
  let (tx, _rx) = mpsc::channel(10);
  let consumer = TokioChannelConsumer::new(tx).with_error_strategy(ErrorStrategy::Skip);
...
#[tokio::test]
async fn test_tokio_channel_consumer_config_operations() {
  let (tx, _rx) = mpsc::channel(10);
  let mut consumer = TokioChannelConsumer::new(tx);
...
#[tokio::test]
async fn test_tokio_channel_consumer_component_info() {
  let (tx, _rx) = mpsc::channel(10);
