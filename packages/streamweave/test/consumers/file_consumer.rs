...
use streamweave::consumers::FileConsumer;
...
async fn test_file_consumer_basic() {
  ...
  let mut consumer = FileConsumer::new(file_path.to_str().unwrap().to_string());
...
async fn test_file_consumer_with_name() {
  let consumer = FileConsumer::new("test.txt".to_string()).with_name("file_writer".to_string());
...
async fn test_file_consumer_with_error_strategy() {
  let consumer = FileConsumer::new("test.txt".to_string()).with_error_strategy(ErrorStrategy::Skip);
...
async fn test_file_consumer_component_info() {
  let consumer = FileConsumer::new("test.txt".to_string()).with_name("test".to_string());

  let info = consumer.component_info();
  assert_eq!(info.name, "test");
  assert!(info.type_name.contains("FileConsumer"));
}
