...
#[tokio::test]
async fn test_tokio_channel_producer_basic() {
...
#[tokio::test]
async fn test_tokio_channel_producer_empty() {
  let (tx, rx) = mpsc::channel(10);
...
#[tokio::test]
async fn test_tokio_channel_producer_with_name() {
  let (_tx, rx) = mpsc::channel(10);
  let producer = TokioChannelProducer::new(rx).with_name("channel_producer".to_string());
...
#[tokio::test]
async fn test_tokio_channel_producer_with_error_strategy() {
  let (_tx, rx) = mpsc::channel(10);
  let producer = TokioChannelProducer::new(rx).with_error_strategy(ErrorStrategy::Skip);
...
#[tokio::test]
async fn test_tokio_channel_producer_config_operations() {
  let (_tx, rx) = mpsc::channel(10);
  let mut producer = TokioChannelProducer::new(rx);
...
#[tokio::test]
async fn test_tokio_channel_producer_component_info() {
  let (_tx, rx) = mpsc::channel(10);
