...
mod tests {
  ...
  proptest! {
      ...
      fn test_identity_law(
          ...
          let arr_f = <Rc<i64> as Category<i64, i64>>::arr(f);

          // Get the identity morphism
          let id = <Rc<i64> as Category<i64, i64>>::id();

          // Compose id . f
          let id_then_f = <Rc<i64> as Category<i64, i64>>::compose(id.clone(), arr_f.clone());

          // Compose f . id
          let f_then_id = <Rc<i64> as Category<i64, i64>>::compose(arr_f.clone(), id);
  ...
  proptest! {
      ...
      fn test_composition_law(
          ...
          let arr_f = <Rc<i64> as Category<i64, i64>>::arr(f);
          let arr_g = <Rc<i64> as Category<i64, i64>>::arr(g);
          let arr_h = <Rc<i64> as Category<i64, i64>>::arr(h);

          // Compose (f . g) . h
          let fg = <Rc<i64> as Category<i64, i64>>::compose(arr_f.clone(), arr_g.clone());
          let fg_h = <Rc<i64> as Category<i64, i64>>::compose(fg, arr_h.clone());

          // Compose f . (g . h)
          let gh = <Rc<i64> as Category<i64, i64>>::compose(arr_g, arr_h);
          let f_gh = <Rc<i64> as Category<i64, i64>>::compose(arr_f, gh);
  ...
  // Test the first combinator - specific to Rc implementation (single-threaded shared ownership)
  proptest! {
      #[test]
      fn test_first_combinator(
          ...
          let first_f = <Rc<i64> as Category<i64, i64>>::first(arr_f);
  ...
  // Test the second combinator - specific to Rc implementation (single-threaded shared ownership)
  proptest! {
      #[test]
      fn test_second_combinator(
          ...
          let second_f = <Rc<i64> as Category<i64, i64>>::second(arr_f);
