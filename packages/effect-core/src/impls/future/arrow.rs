...
mod tests {
  ...
  async fn test_split() {
    ...
    let split =
      <FutureCategory<i64> as Arrow<i64, i64>>::split::<i64, i64, (), ()>(add_one, double);
  ...
  async fn test_split_law() {
    ...
    let split =
      <FutureCategory<i64> as Arrow<i64, i64>>::split::<i64, i64, (), ()>(arrow_f, arrow_g);

    let direct =
      <FutureCategory<(i64, i64)> as Arrow<(i64, i64), (i64, i64)>>::arrow(move |(x, y)| {
        (f(x), g(y))
      });
