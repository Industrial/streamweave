...
use streamweave_path::ParentPathTransformer;
...
async fn test_parent_path_transformer_with_name() {
  let transformer = ParentPathTransformer::new().with_name("test-transformer".to_string());
  assert_eq!(
...
async fn test_parent_path_transformer_default() {
  let transformer = ParentPathTransformer::default();
  assert_eq!(transformer.config().name(), None);
}

#[tokio::test]
async fn test_parent_path_transformer_clone() {
  let transformer1 = ParentPathTransformer::new().with_name("original".to_string());
...
async fn test_parent_path_transformer_with_error_strategy() {
  ...
  let transformer = ParentPathTransformer::new().with_error_strategy(ErrorStrategy::Skip);
  assert!(matches!(
...
async fn test_parent_path_transformer_handle_error_stop() {
  ...
  let transformer = ParentPathTransformer::new().with_error_strategy(ErrorStrategy::Stop);
...
async fn test_parent_path_transformer_handle_error_retry() {
  ...
  let transformer = ParentPathTransformer::new().with_error_strategy(ErrorStrategy::Retry(3));
...
async fn test_parent_path_transformer_handle_error_custom() {
  ...
  let transformer = ParentPathTransformer::new()
...
async fn test_parent_path_transformer_create_error_context() {
  ...
  assert!(context.component_type.contains("ParentPathTransformer"));
}
...
async fn test_parent_path_transformer_create_error_context_default_name() {
  ...
  assert_eq!(context.component_name, "parent_path_transformer");
...
async fn test_parent_path_transformer_component_info() {
  ...
  assert!(info.type_name.contains("ParentPathTransformer"));
}
...
async fn test_parent_path_transformer_component_info_default_name() {
  ...
  assert_eq!(info.name, "parent_path_transformer");
}
