  ...
  B: InferenceBackend + 'static,
{
  ...
  ///
  /// ```rust,no_run
  /// # use streamweave_ml_transformers::{InferenceTransformer, OnnxBackend, swap_model};
  /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
  /// let mut backend = OnnxBackend::new()?;
  /// backend.load_from_path("model.onnx").await?;
  ///
  ...
  /// # }
  /// ```
  pub async fn swap_backend(&mut self, new_backend: B) -> Result<(), super::SwapError> {
    super::swap_model(&self.backend, new_backend).await
  }
}

// Transformer trait implementation
mod transformer_impl {
  use super::{InferenceBackend, InferenceTransformer};
