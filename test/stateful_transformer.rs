...
use streamweave::stateful_transformer::*;
...
fn test_checkpoint_error_display() {
  assert_eq!(
    format!("{}", StatefulCheckpointError::NoState),
    "No state to checkpoint"
  );
  assert_eq!(
    format!(
      "{}",
      StatefulCheckpointError::SerializationFailed("bad".to_string())
    ),
  ...
  assert_eq!(
    format!(
      "{}",
      StatefulCheckpointError::DeserializationFailed("bad".to_string())
    ),
  ...
  assert_eq!(
    format!(
      "{}",
      StatefulCheckpointError::IoError(std::io::Error::new(
  ...
  assert_eq!(
    format!(
      "{}",
      StatefulCheckpointError::StateError(StateError::NotInitialized)
    ),
...
fn test_checkpoint_error_source() {
  let io_err = std::io::Error::new(std::io::ErrorKind::NotFound, "test");
  let checkpoint_err = StatefulCheckpointError::IoError(io_err);
...
fn test_file_checkpoint_store_load_nonexistent() {
  ...
  let store = FileStatefulCheckpointStore::new(path);
...
fn test_in_memory_checkpoint_store_default() {
  let store = InMemoryStatefulCheckpointStore::default();
  assert!(!store.exists());
}
...
fn test_restore_from_json_checkpoint_invalid() {
  ...
  assert!(matches!(
    result.unwrap_err(),
    StatefulCheckpointError::DeserializationFailed(_)
  ));
}
...
fn test_save_checkpoint() {
  ...
  let temp_dir = TempDir::new().unwrap();
  let path = temp_dir.path().join("checkpoint.json");
  let file_store = FileStatefulCheckpointStore::new(path);
...
fn test_checkpoint_manager_new() {
  ...
  let config = StatefulCheckpointConfig::default();
  let manager: StatefulCheckpointManager<i64> = StatefulCheckpointManager::new(store, config);
...
fn test_checkpoint_manager_with_file() {
  ...
  let path = temp_dir.path().join("checkpoint.json");
  let config = StatefulCheckpointConfig::default();
  let manager: StatefulCheckpointManager<i64> = StatefulCheckpointManager::with_file(path, config);
  assert!(!manager.has_checkpoint());
}
...
fn test_checkpoint_manager_config() {
  let store = Box::new(InMemoryStatefulCheckpointStore::new());
  let config = StatefulCheckpointConfig::with_interval(100);
  let manager: StatefulCheckpointManager<i64> =
    StatefulCheckpointManager::new(store, config.clone());
