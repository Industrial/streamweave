...
async fn test_file_producer_basic() {
  ...
  let mut producer = FsFileProducer::new(file_path.to_str().unwrap().to_string());
...
async fn test_file_producer_with_name() {
  let producer = FsFileProducer::new("test.txt".to_string()).with_name("file_reader".to_string());
...
async fn test_file_producer_with_error_strategy() {
  let producer =
    FsFileProducer::new("test.txt".to_string()).with_error_strategy(ErrorStrategy::Skip);
...
async fn test_file_producer_component_info() {
  let producer = FsFileProducer::new("test.txt".to_string()).with_name("test".to_string());

  let info = producer.component_info();
  assert_eq!(info.name, "test");
  assert!(info.type_name.contains("FsFileProducer"));
}
