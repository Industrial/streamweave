//! Comprehensive tests for DbSqliteProducer
...
use streamweave::db::{DatabaseProducerConfig, DatabaseRow, DatabaseType};
...
async fn test_sqlite_producer_new() {
  ...
  let producer = DbSqliteProducer::new(config.clone());
...
async fn test_sqlite_producer_with_error_strategy() {
  ...
  let producer = DbSqliteProducer::new(config).with_error_strategy(ErrorStrategy::Skip);
...
async fn test_sqlite_producer_with_name() {
  ...
  let producer = DbSqliteProducer::new(config).with_name("test_producer".to_string());
...
async fn test_sqlite_producer_clone() {
  ...
  let producer = DbSqliteProducer::new(config).with_name("test".to_string());
...
async fn test_sqlite_producer_produce_simple_query() {
  ...
  let mut producer = DbSqliteProducer::new(config);
