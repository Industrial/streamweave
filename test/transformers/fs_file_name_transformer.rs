...
async fn test_file_name_transformer_basic() {
  let mut transformer = FsFileNameTransformer::new();
...
async fn test_file_name_transformer_with_name() {
  let transformer = FsFileNameTransformer::new().with_name("test-transformer".to_string());
  assert_eq!(
...
async fn test_file_name_transformer_default() {
  let transformer = FsFileNameTransformer::default();
  assert_eq!(transformer.config().name(), None);
}

#[tokio::test]
async fn test_file_name_transformer_clone() {
  let transformer1 = FsFileNameTransformer::new().with_name("original".to_string());
...
async fn test_file_name_transformer_with_error_strategy() {
  ...
  let transformer = FsFileNameTransformer::new().with_error_strategy(ErrorStrategy::Skip);
  assert!(matches!(
...
async fn test_file_name_transformer_handle_error_stop() {
  ...
  let transformer = FsFileNameTransformer::new().with_error_strategy(ErrorStrategy::Stop);
...
async fn test_file_name_transformer_handle_error_retry() {
  ...
  let transformer = FsFileNameTransformer::new().with_error_strategy(ErrorStrategy::Retry(3));
...
async fn test_file_name_transformer_create_error_context() {
  ...
  assert!(context.component_type.contains("FsFileNameTransformer"));
}
...
async fn test_file_name_transformer_create_error_context_default_name() {
  ...
  assert_eq!(context.component_name, "fs_file_name_transformer");
...
async fn test_file_name_transformer_component_info() {
  ...
  assert!(info.type_name.contains("FsFileNameTransformer"));
}
...
async fn test_file_name_transformer_component_info_default_name() {
  ...
  assert_eq!(info.name, "fs_file_name_transformer");
}
