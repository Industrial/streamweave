...
async fn test_normalize_path_transformer_basic() {
  let mut transformer = FsNormalizePathTransformer::new();
...
async fn test_normalize_path_transformer_with_name() {
  let transformer = FsNormalizePathTransformer::new().with_name("test-transformer".to_string());
  assert_eq!(
...
async fn test_normalize_path_transformer_default() {
  let transformer = FsNormalizePathTransformer::default();
  assert_eq!(transformer.config().name(), None);
}

#[tokio::test]
async fn test_normalize_path_transformer_clone() {
  let transformer1 = FsNormalizePathTransformer::new().with_name("original".to_string());
...
async fn test_normalize_path_transformer_with_error_strategy() {
  ...
  let transformer = FsNormalizePathTransformer::new().with_error_strategy(ErrorStrategy::Skip);
  assert!(matches!(
...
async fn test_normalize_path_transformer_handle_error_stop() {
  ...
  let transformer = FsNormalizePathTransformer::new().with_error_strategy(ErrorStrategy::Stop);
...
async fn test_normalize_path_transformer_handle_error_retry() {
  ...
  let transformer = FsNormalizePathTransformer::new().with_error_strategy(ErrorStrategy::Retry(3));
...
async fn test_normalize_path_transformer_create_error_context() {
  ...
  assert!(
    context
      .component_type
      .contains("FsNormalizePathTransformer")
  );
}
...
async fn test_normalize_path_transformer_create_error_context_default_name() {
  ...
  assert_eq!(context.component_name, "fs_normalize_path_transformer");
...
async fn test_normalize_path_transformer_component_info() {
  ...
  assert!(info.type_name.contains("FsNormalizePathTransformer"));
}
...
async fn test_normalize_path_transformer_component_info_default_name() {
  ...
  assert_eq!(info.name, "fs_normalize_path_transformer");
}
