...
/// Creates and runs an advanced pipeline that processes data through multiple transformers
pub async fn run_advanced_pipeline() -> Result<(), Box<dyn std::error::Error>> {
...
/// Creates a pipeline with custom batch size
pub fn create_batch_pipeline(
  input_file: &str,
  output_file: &str,
  batch_size: usize,
) -> Result<(), Box<dyn std::error::Error>> {
  let batch_transformer = BatchTransformer::new(batch_size)?;

  PipelineBuilder::new()
    .producer(FileProducer::new(input_file.to_string()))
    .transformer(MapTransformer::new(|s: String| {
      s.parse::<i32>().unwrap_or_default()
    }))
    ...
    ._consumer(
      FileConsumer::new(output_file.to_string()).with_error_strategy(ErrorStrategy::<String>::Stop),
    );

  Ok(())
}

/// Creates a pipeline with rate limiting and circuit breaker
pub fn create_resilient_pipeline(input_file: &str, output_file: &str) {
  PipelineBuilder::new()
    .producer(FileProducer::new(input_file.to_string()))
    .transformer(MapTransformer::new(|s: String| {
      s.parse::<i32>().unwrap_or_default()
    }))
    .transformer(BatchTransformer::new(2).unwrap())
    .transformer(MapTransformer::new(|batch: Vec<i32>| {
      batch
        .iter()
        .map(|i| i.to_string())
    ...
    .transformer(RateLimitTransformer::new(1, Duration::from_millis(10)))
    .transformer(CircuitBreakerTransformer::new(3, Duration::from_secs(1)))
    .transformer(RetryTransformer::new(3, Duration::from_millis(10)))
    ._consumer(
      FileConsumer::new(output_file.to_string()).with_error_strategy(ErrorStrategy::<String>::Stop),
    );
}
