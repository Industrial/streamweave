...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    graph1.expose_output_port("transform", "out", "result").map_err(|e| format!("Failed to expose output: {}", e))?;
    ...
    graph2.expose_output_port("transform", "out", "output").map_err(|e| format!("Failed to expose output: {}", e))?;
