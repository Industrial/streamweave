...
use std::sync::Arc;
use std::sync::Mutex;
use streamweave::checkpoint::{CheckpointId, CheckpointStorage, FileCheckpointStorage};
...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let node = StatefulNode::new(
        "stateful".to_string(),
        b"hello-checkpoint".to_vec(),
    );
    graph.add_node("stateful".to_string(), Box::new(node)).unwrap();
    ...
    println!("List checkpoints: {:?}", ids.iter().map(|id| id.as_u64()).collect::<Vec<_>>());
