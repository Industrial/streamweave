...
//!
//! Graph is loaded from [checkpoint_restore.mmd](checkpoint_restore.mmd).
//!
...
mod run {
  ...
  pub fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let mmd_path = Path::new("examples/checkpoint_restore.mmd");
    let bp = parse_mmd_file_to_blueprint(mmd_path)?;
    let mut registry = NodeRegistry::new();
    registry.register("StatefulNode", |id, _inputs, _outputs| {
      Box::new(StatefulNode::new(id, b"hello-checkpoint".to_vec()))
    });
    ...
    println!(
      "List checkpoints: {:?}",
      ids.iter().map(|i| i.as_u64()).collect::<Vec<_>>()
    );

    let mut registry2 = NodeRegistry::new();
    registry2.register("StatefulNode", |id, _inputs, _outputs| {
      Box::new(StatefulNode::new(id, vec![]))
    });
    let mut graph2 = blueprint_to_graph(&bp, Some(&registry2))?;
