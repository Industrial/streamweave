...
//!
//! Graph is loaded from [production_ready.mmd](production_ready.mmd).
//!
...
mod run {
  ...
  pub async fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!(
      "Before execute: is_ready={}, is_live={}",
      graph.is_ready(),
      graph.is_live()
    );
    ...
    println!(
      "After execute: is_ready={}, is_live={}",
      graph.is_ready(),
      graph.is_live()
    );
    ...
    println!(
      "After wait_for_completion: is_ready={}, is_live={}",
      graph.is_ready(),
      graph.is_live()
    );
