...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    graph
        .add_node("n".to_string(), Box::new(VariableNode::new("n".to_string())))
        .unwrap();
    ...
    println!("ShardConfig: shard_id={}, total_shards={}", config.shard_id, config.total_shards);
    ...
    println!("\nExport state for keys [alice, bob]: {} bytes (node has no key-scoped state)", data.len());
