...
async fn run_once() -> Result<Vec<i32>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    graph
        .add_node("a".to_string(), Box::new(ProducerNode::new("a".to_string(), vec![1, 2, 3])))
        .unwrap();
    graph
        .add_node("b".to_string(), Box::new(ProducerNode::new("b".to_string(), vec![10, 20])))
        .unwrap();
    graph
        .add_node(
            "merge".to_string(),
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    println!("Deterministic execution example (execute_deterministic, MergeNode::new_deterministic)\n");
