...
impl Node for FailNTimesThenSucceedNode {
    ...
    > {
        ...
        Box::pin(async move {
            ...
            if c < fail_count {
                return Err(format!("Failing attempt {} (will succeed after {} failures)", c + 1, fail_count).into());
            }
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    graph
        .add_node("producer".to_string(), Box::new(ProducerNode::new("producer".to_string(), vec![1, 2, 3])))
        .unwrap();
    graph
        .add_node(
            "flaky".to_string(),
    ...
    assert_eq!(received, vec![1, 2, 3], "after restart, all items should pass through");
    println!("\nDone.");
    Ok(())
}
