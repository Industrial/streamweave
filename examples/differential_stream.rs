  ...
  > {
    ...
    Box::pin(async move {
      ...
      tokio::spawn(async move {
        for kv in data {
          let _ = tx.send(Arc::new(kv) as Arc<dyn Any + Send + Sync>).await;
        }
      });
...
async fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  while let Some(arc) = out_rx.recv().await {
    if let Ok(msg) = arc
      ...
      && let DifferentialStreamMessage::Data(elem) = msg.as_ref()
      && let Ok(pair) = elem.payload().clone().downcast::<(String, i64)>()
    {
  ...
  println!(
    "  Group-by output (key, count, time, diff): {:?}",
    group_by_results
  );
  println!();
  ...
  println!(
    "  Join output count: {} (payloads are HashMap with left/right)",
    join_results.len()
  );
  println!();
