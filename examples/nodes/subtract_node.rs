...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph = Graph::new("subtract_example".to_string());
  graph.add_node(
    "subtract".to_string(),
    Box::new(SubtractNode::new("subtract".to_string())),
  )?;
  graph.expose_input_port("subtract", "configuration", "configuration")?;
  graph.expose_input_port("subtract", "in1", "in1")?;
  graph.expose_input_port("subtract", "in2", "in2")?;
  graph.expose_output_port("subtract", "out", "output")?;
  graph.expose_output_port("subtract", "error", "error")?;
  ...
  println!("✓ Graph built with SubtractNode using Graph API");
