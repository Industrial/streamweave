...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph: Graph = graph! {
    modulo: ModuloNode::new("modulo".to_string()),
    graph.configuration => modulo.configuration,
    graph.in1 => modulo.in1,
    graph.in2 => modulo.in2,
    modulo.out => graph.output,
    modulo.error => graph.error
  };
  ...
  println!("✓ Graph built with ModuloNode using graph! macro");
