...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph: Graph = {
    ...
    let path = Path::new("examples/nodes/math/max_node.mmd");
    let bp = parse_mmd_file_to_blueprint(path).map_err(|e| e.to_string())?;
    let mut registry = NodeRegistry::new();
    registry.register("MaxNode", |id, _inputs, _outputs| {
      Box::new(MaxNode::new(id))
    });
  ...
  // Test maximum computations
  println!("\n🧪 Testing maximum computations...");

  // Test 1: max(5, 3) = 5
  println!("  Test 1: max(5, 3) (expected: 5)");
  let _ = in1_tx
  ...
  // Test 2: max(2.5, 7.8) = 7.8
  println!("  Test 2: max(2.5, 7.8) (expected: 7.8)");
  let _ = in1_tx
  ...
  // Test 3: max(-10, -3) = -3
  println!("  Test 3: max(-10, -3) (expected: -3)");
  let _ = in1_tx
  ...
  // Test 4: max(0, 0) = 0
  println!("  Test 4: max(0, 0) (expected: 0)");
  let _ = in1_tx
  ...
  // Execute the graph
  println!("Executing graph with MaxNode...");
  ...
  loop {
    ...
    if let Ok(Some(item)) = out_result {
      // MaxNode outputs the maximum value
      results_received += 1;
      println!("  Maximum result {}:", results_received);
  ...
  // Verify behavior: should receive 4 results (5, 7.8, -3, 0)
  if success_count == 4 && error_count == 0 {
    println!("✓ MaxNode correctly computed maximum values");
    println!("  Results should be: [5, 7.8, -3, 0]");
  } else {
    println!(
      "⚠ MaxNode behavior may be unexpected (successes: {}, errors: {}, expected successes: 4, errors: 0)",
