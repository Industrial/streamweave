...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph: Graph = graph! {
    floor: FloorNode::new("floor".to_string()),
    graph.configuration => floor.configuration,
    graph.input => floor.in,
    floor.out => graph.output,
    floor.error => graph.error
  };
  ...
  println!("✓ Graph built with FloorNode using graph! macro");
