...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph: Graph = {
    ...
    let path = Path::new("examples/nodes/stream/distinct_node.mmd");
    let bp = parse_mmd_file_to_blueprint(path).map_err(|e| e.to_string())?;
    let mut registry = NodeRegistry::new();
    registry.register("DistinctNode", |id, _inputs, _outputs| {
      Box::new(DistinctNode::new(id))
    });
