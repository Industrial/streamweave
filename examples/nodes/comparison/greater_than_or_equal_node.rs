...
use streamweave::nodes::comparison::GreaterThanOrEqualNode;
use tokio::sync::mpsc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph: Graph = graph! {
    greater_than_or_equal: GreaterThanOrEqualNode::new("greater_than_or_equal".to_string()),
    graph.configuration => greater_than_or_equal.configuration,
    graph.in1 => greater_than_or_equal.in1,
    graph.in2 => greater_than_or_equal.in2,
    greater_than_or_equal.out => graph.output,
    greater_than_or_equal.error => graph.error
  };
  ...
  println!("✓ Graph built with GreaterThanOrEqualNode using graph! macro");

  // Send configuration (optional for GreaterThanOrEqualNode)
  let _ = config_tx
    .send(Arc::new(()) as Arc<dyn Any + Send + Sync>)
    .await;

  // Test greater than or equal comparisons
  println!("\n🧪 Testing greater than or equal comparisons...");
  ...
  // Test 4: 5.5 >= 3.2 (expected: true)
  println!("  Test 4: 5.5f64 >= 3.2f64 (expected: true)");
  let _ = in1_tx
  ...
  // Test 5: 3.2 >= 5.5 (expected: false)
  println!("  Test 5: 3.2f64 >= 5.5f64 (expected: false)");
  let _ = in1_tx
    .send(Arc::new(3.2f64) as Arc<dyn Any + Send + Sync>)
    .await;
  let _ = in2_tx
  ...
  // Execute the graph
  println!("Executing graph with GreaterThanOrEqualNode...");
  ...
  loop {
    ...
    if let Ok(Some(item)) = out_result {
      // GreaterThanOrEqualNode outputs the boolean result of greater than or equal comparison
  ...
  if success_count == 5 && error_count == 0 {
    println!("✓ GreaterThanOrEqualNode correctly performed greater than or equal comparisons");
    println!("  Results should match expected values: [true, false, true, true, false]");
  } else {
    println!(
      "⚠ GreaterThanOrEqualNode behavior may be unexpected (successes: {}, errors: {}, expected successes: 5, errors: 0)",
