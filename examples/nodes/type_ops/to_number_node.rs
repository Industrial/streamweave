...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph: Graph = graph! {
    to_number: ToNumberNode::new("to_number".to_string()),
    graph.configuration => to_number.configuration,
    graph.input => to_number.in,
    to_number.out => graph.output,
    to_number.error => graph.error
  };
  ...
  println!("✓ Graph built with ToNumberNode using graph! macro");
