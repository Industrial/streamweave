...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph = Graph::new("ends_with_example".to_string());
  graph.add_node(
    "ends_with".to_string(),
    Box::new(StringEndsWithNode::new("ends_with".to_string())),
  )?;
  graph.expose_input_port("ends_with", "configuration", "configuration")?;
  graph.expose_input_port("ends_with", "in", "input")?;
  graph.expose_input_port("ends_with", "suffix", "suffix")?;
  graph.expose_output_port("ends_with", "out", "output")?;
  graph.expose_output_port("ends_with", "error", "error")?;
  ...
  println!("✓ Graph built with StringEndsWithNode using Graph API");
