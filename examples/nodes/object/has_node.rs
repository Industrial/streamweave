...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph = Graph::new("has_example".to_string());
  graph.add_node(
    "has".to_string(),
    Box::new(ObjectHasPropertyNode::new("has".to_string())),
  )?;
  graph.expose_input_port("has", "configuration", "configuration")?;
  graph.expose_input_port("has", "in", "input")?;
  graph.expose_input_port("has", "key", "key")?;
  graph.expose_output_port("has", "out", "output")?;
  graph.expose_output_port("has", "error", "error")?;
  ...
  println!("✓ Graph built with ObjectHasPropertyNode (has) using Graph API");
  ...
  obj3.insert("active".to_string(), Arc::new(true) as Arc<dyn Any + Send + Sync>);
  ...
  obj4.insert("name".to_string(), Arc::new("Charlie".to_string()) as Arc<dyn Any + Send + Sync>);
  obj4.insert("age".to_string(), Arc::new(35i64) as Arc<dyn Any + Send + Sync>);
  let _ = in_tx
  ...
  if success_count == 4 && error_count == 0 {
    ...
    println!("  Results should be: true, false, true, false (checking for existing/non-existing properties)");
  } else {
    println!(
