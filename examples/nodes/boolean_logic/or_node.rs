...
use streamweave::nodes::boolean_logic::OrNode;
use tokio::sync::mpsc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph: Graph = {
    ...
    let path = Path::new("examples/nodes/boolean_logic/or_node.mmd");
    let bp = parse_mmd_file_to_blueprint(path).map_err(|e| e.to_string())?;
    let mut registry = NodeRegistry::new();
    registry.register("OrNode", |id, _inputs, _outputs| Box::new(OrNode::new(id)));
  ...
  println!("✓ Graph built with OrNode using graph! macro");

  // Send configuration (optional for OrNode)
  let _ = config_tx
    .send(Arc::new(()) as Arc<dyn Any + Send + Sync>)
    .await;

  // Send test data: boolean pairs to OR
  let test_pairs = vec![
  ...
  // Execute the graph
  println!("Executing graph with OrNode...");
  ...
  loop {
    ...
    if let Ok(Some(item)) = out_result {
      // OrNode outputs the boolean result of OR operation
  ...
  // Verify behavior: should receive 4 results (true, true, true, false)
  if success_count == 4 && error_count == 0 {
    println!("✓ OrNode correctly performed logical OR operations");
  } else {
    println!(
      "⚠ OrNode behavior may be unexpected (successes: {}, errors: {}, expected successes: 4, errors: 0)",
