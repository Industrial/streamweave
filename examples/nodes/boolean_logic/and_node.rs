...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  graph.expose_output_port("and", "out", "output")?;
  graph.expose_output_port("and", "error", "error")?;
  ...
  println!("✓ Graph built with AndNode using Graph API");

  // Send configuration (optional for AndNode)
  let _ = config_tx
    .send(Arc::new(()) as Arc<dyn Any + Send + Sync>)
    .await;

  // Send test data: boolean pairs to AND
  let test_pairs = vec![
  ...
  // Execute the graph
  println!("Executing graph with AndNode...");
  ...
  loop {
    ...
    if let Ok(Some(item)) = out_result {
      // AndNode outputs the boolean result of AND operation
  ...
  // Verify behavior: should receive 4 results (true, false, false, false)
  if success_count == 4 && error_count == 0 {
    println!("✓ AndNode correctly performed logical AND operations");
  } else {
    println!(
      "⚠ AndNode behavior may be unexpected (successes: {}, errors: {}, expected successes: 4, errors: 0)",
