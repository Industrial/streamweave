...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph: Graph = {
    ...
    let path = Path::new("examples/nodes/boolean_logic/nor_node.mmd");
    let bp = parse_mmd_file_to_blueprint(path).map_err(|e| e.to_string())?;
    let mut registry = NodeRegistry::new();
    registry.register("NorNode", |id, _inputs, _outputs| {
      Box::new(NorNode::new(id))
    });
  ...
  // Send test data: boolean pairs to NOR
  let test_pairs = vec![
    (true, true),   // true NOR true = false (NOT (true OR true))
    (true, false),  // true NOR false = false (NOT (true OR false))
    (false, true),  // false NOR true = false (NOT (false OR true))
    (false, false), // false NOR false = true (NOT (false OR false))
  ];
  ...
  // Execute the graph
  println!("Executing graph with NorNode...");
  ...
  loop {
    ...
    if let Ok(Some(item)) = out_result {
      // NorNode outputs the boolean result of NOR operation
      if let Ok(result_arc) = item.clone().downcast::<bool>() {
        let result = *result_arc;
        println!("  NOR result: {}", result);
        success_count += 1;
        has_data = true;
      }
    }
  ...
  // Verify behavior: should receive 4 results (false, false, false, true)
  if success_count == 4 && error_count == 0 {
    println!("✓ NorNode correctly performed logical NOR operations");
  } else {
    println!(
      "⚠ NorNode behavior may be unexpected (successes: {}, errors: {}, expected successes: 4, errors: 0)",
