...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph = Graph::new("nand_example".to_string());
  graph.add_node(
    "nand".to_string(),
    Box::new(NandNode::new("nand".to_string())),
  )?;
  graph.expose_input_port("nand", "configuration", "configuration")?;
  graph.expose_input_port("nand", "in1", "in1")?;
  graph.expose_input_port("nand", "in2", "in2")?;
  graph.expose_output_port("nand", "out", "output")?;
  graph.expose_output_port("nand", "error", "error")?;
  ...
  println!("✓ Graph built with NandNode using Graph API");
