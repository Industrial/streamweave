...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut graph: Graph = {
    ...
    let path = Path::new("examples/nodes/aggregation/max_aggregate_node.mmd");
    let bp = parse_mmd_file_to_blueprint(path).map_err(|e| e.to_string())?;
    let mut registry = NodeRegistry::new();
    registry.register("MaxAggregateNode", |id, _inputs, _outputs| {
      Box::new(MaxAggregateNode::new(id))
    });
  ...
  println!("✓ Graph built with MaxAggregateNode using graph! macro");
