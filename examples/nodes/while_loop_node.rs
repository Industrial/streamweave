...
use streamweave::nodes::while_loop_node::{while_loop_config, WhileLoopConfig, WhileLoopNode};
use tokio::sync::mpsc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    graph.add_node("while_loop".to_string(), Box::new(WhileLoopNode::new("while_loop".to_string())))?;
    ...
    loop {
        ...
        let break_result = tokio::time::timeout(tokio::time::Duration::from_millis(100), break_out_rx.recv()).await;
    ...
    println!("✓ Received {} items that completed normally via output channel", completed_count);
    println!("✓ Received {} items that broke early via break channel", broken_count);
