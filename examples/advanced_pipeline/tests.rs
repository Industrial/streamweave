...
async fn test_advanced_pipeline_processes_data_correctly() {
  ...
  let mut map_transformer1 = MapTransformer::new(|s: String| s.parse::<i32>().unwrap_or_default());
  ...
  map_transformer2
    .set_config(TransformerConfig::default().with_name("batch_to_string".to_string()));
