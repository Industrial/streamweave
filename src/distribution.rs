...
impl ShardedRunner {
    ...
    {
        Self::new_with_capacity(total_shards, partitioning, factory, input_port, output_port, 64)
    }
...
async fn router_task(
    ...
    while let Some(payload) = input_rx.recv().await {
        let shard_id = match partitioning.key_extractor.extract_key(payload.clone()).await {
...
mod tests {
    ...
    async fn test_sharded_runner_new_and_shard_count() {
        let partitioning: PartitioningConfig = partition_by_key(
            |p: Arc<dyn std::any::Any + Send + Sync>| {
                Box::pin(async move {
