//! # Not Equal Node
//!
//! A transform node that performs inequality comparison on two inputs.
//!
//! ## Ports
//!
...
//! ## Behavior
//!
//! The node performs inequality comparison on two values. It supports:
...
//! - Incompatible types: returns true (no error) for incompatible type combinations

use crate::graph::node::{InputStreams, Node, NodeExecutionError, OutputStreams};
use crate::graph::nodes::common::BaseNode;
use crate::graph::nodes::comparison::common::compare_not_equal;
...
/// A node that performs inequality comparison on two inputs.
///
/// The node receives two values on "in1" and "in2" ports and outputs
/// the result of the inequality comparison (as a boolean) to the "out" port.
pub struct NotEqualNode {
  pub(crate) base: BaseNode,
}

impl NotEqualNode {
  /// Creates a new NotEqualNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::comparison::NotEqualNode;
  ///
  /// let node = NotEqualNode::new("not_equal".to_string());
