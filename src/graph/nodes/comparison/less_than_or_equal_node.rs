...
//!
//! A transform node that performs less than or equal comparison on two inputs.
//!
//! ## Ports
//!
...
//! ## Behavior
//!
//! The node performs less than or equal comparison on two values. It supports:
...
use crate::graph::nodes::comparison::common::compare_less_than_or_equal;
...
/// A node that performs less than or equal comparison on two inputs.
///
/// The node receives two values on "in1" and "in2" ports and outputs
/// the result of the less than or equal comparison (as a boolean) to the "out" port.
pub struct LessThanOrEqualNode {
  pub(crate) base: BaseNode,
}

impl LessThanOrEqualNode {
  /// Creates a new LessThanOrEqualNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::comparison::LessThanOrEqualNode;
  ///
  /// let node = LessThanOrEqualNode::new("less_than_or_equal".to_string());
...
#[async_trait]
impl Node for LessThanOrEqualNode {
  fn name(&self) -> &str {
    self.base.name()
  }
      ...
      tokio::spawn(async move {
        ...
        while let Some((port, item)) = merged.next().await {
          ...
          if let (Some(v1), Some(v2)) = (in1_buffer.as_ref(), in2_buffer.as_ref()) {
            match compare_less_than_or_equal(v1, v2) {
              Ok(result) => {
