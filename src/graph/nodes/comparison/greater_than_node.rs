//! # Greater Than Node
//!
//! A transform node that performs greater than comparison on two inputs.
//!
//! ## Ports
//!
...
//! ## Behavior
//!
//! The node performs greater than comparison on two values. It supports:
...
/// A node that performs greater than comparison on two inputs.
///
/// The node receives two values on "in1" and "in2" ports and outputs
/// the result of the greater than comparison (as a boolean) to the "out" port.
pub struct GreaterThanNode {
  pub(crate) base: BaseNode,
}

impl GreaterThanNode {
  /// Creates a new GreaterThanNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::comparison::GreaterThanNode;
  ///
  /// let node = GreaterThanNode::new("greater_than".to_string());
      ...
      tokio::spawn(async move {
        ...
        while let Some((port, item)) = merged.next().await {
          ...
          if let (Some(v1), Some(v2)) = (in1_buffer.as_ref(), in2_buffer.as_ref()) {
            match compare_greater_than(v1, v2) {
              Ok(result) => {
