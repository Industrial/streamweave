...
//!
//! A transform node that performs greater than or equal comparison on two inputs.
//!
//! ## Ports
//!
...
//! ## Behavior
//!
//! The node performs greater than or equal comparison on two values. It supports:
...
use crate::graph::nodes::comparison::common::compare_greater_than_or_equal;
...
/// A node that performs greater than or equal comparison on two inputs.
///
...
impl GreaterThanOrEqualNode {
  /// Creates a new GreaterThanOrEqualNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::comparison::GreaterThanOrEqualNode;
  ///
  /// let node = GreaterThanOrEqualNode::new("greater_than_or_equal".to_string());
...
#[async_trait]
impl Node for GreaterThanOrEqualNode {
  fn name(&self) -> &str {
    self.base.name()
  }
      ...
      tokio::spawn(async move {
        ...
        while let Some((port, item)) = merged.next().await {
          ...
          if let (Some(v1), Some(v2)) = (in1_buffer.as_ref(), in2_buffer.as_ref()) {
            match compare_greater_than_or_equal(v1, v2) {
              Ok(result) => {
