//! # Skip Node
//!
//! A transform node that skips the first N items from the input stream.
//!
//! ## Ports
//!
//! - **Input**: `"configuration"` - Receives configuration (currently unused, for consistency)
//! - **Input**: `"in"` - Receives data items
//! - **Input**: `"count"` - Receives count value (numeric: how many items to skip)
//! - **Output**: `"out"` - Sends items after skipping the first N
//! - **Output**: `"error"` - Sends errors that occur during processing
//!
//! ## Behavior
//!
//! The node skips the first N items from the input stream and forwards the remaining items to the output.
//! It supports:
//! - Skipping items based on a numeric count (usize, i32, i64, u32, u64)
...
/// A node that skips the first N items from the input stream.
///
...
impl SkipNode {
  /// Creates a new SkipNode with the given name.
  ///
  /// # Arguments
  ///
  ...
  /// # Returns
  ///
  /// A new `SkipNode` instance.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::stream::SkipNode;
  ///
  /// let node = SkipNode::new("skip".to_string());
      ...
      tokio::spawn(async move {
        ...
        while let Some((port, item)) = merged_stream.next().await {
          match port {
            ...
            InputPort::In => {
              // Skip items until we've skipped the count, then forward
              if let Some(c) = count {
                if items_skipped < c {
                  // Skip this item
                  items_skipped += 1;
