//! # Minimum Node
//!
//! A transform node that computes the minimum of two numeric inputs.
//!
//! ## Ports
//!
...
//! - **Output**: `"out"` - Sends the minimum of the two inputs
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., type mismatch)
//!
//! ## Behavior
//!
//! The node computes the minimum of two numeric inputs. It supports:
...
use crate::graph::nodes::math::common::min_values;
...
/// A node that computes the minimum of two numeric inputs.
///
/// The node receives two numeric values on "in1" and "in2" ports and outputs
/// the minimum value to the "out" port.
pub struct MinNode {
  pub(crate) base: BaseNode,
}

impl MinNode {
  /// Creates a new MinNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::math::MinNode;
  ///
  /// let node = MinNode::new("min".to_string());
