...
#[tokio::test]
async fn test_is_string_node_creation() {
  use crate::graph::nodes::type_ops::IsStringNode;
  let node = IsStringNode::new("test_is_string".to_string());
  assert_eq!(node.name(), "test_is_string");
...
#[tokio::test]
async fn test_is_string_string() {
  use crate::graph::nodes::type_ops::IsStringNode;
  let node = IsStringNode::new("test_is_string".to_string());
  ...
  if let Ok(is_str) = results[0].clone().downcast::<bool>() {
    assert!(*is_str);
  } else {
...
#[tokio::test]
async fn test_is_string_object() {
  use crate::graph::nodes::type_ops::IsStringNode;
  let node = IsStringNode::new("test_is_string".to_string());
...
#[tokio::test]
async fn test_is_string_multiple_types() {
  use crate::graph::nodes::type_ops::IsStringNode;
  let node = IsStringNode::new("test_is_string".to_string());
  ...
  // Second item: i32 (not string) -> false
  if let Ok(is_str) = results[1].clone().downcast::<bool>() {
    assert!(!*is_str);
  } else {
    panic!("Second result is not a bool");
  }

  // Third item: bool (not string) -> false
  if let Ok(is_str) = results[2].clone().downcast::<bool>() {
    assert!(!*is_str);
  } else {
