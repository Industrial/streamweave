//! # String Equal Node
//!
//! A transform node that checks if two strings are equal.
//!
//! ## Ports
//!
//! - **Input**: `"configuration"` - Receives configuration (currently unused, for consistency)
//! - **Input**: `"in1"` - Receives first string value
//! - **Input**: `"in2"` - Receives second string value
//! - **Output**: `"out"` - Sends boolean result (true if equal, false otherwise)
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., type mismatch)
//!
//! ## Behavior
//!
//! The node checks if two strings are equal and returns a boolean. It supports:
...
use crate::graph::nodes::string::common::string_equal;
...
/// Enum to tag input ports for internal merging.
enum InputPort {
  In1,
  In2,
}

/// A node that checks if two strings are equal.
///
/// The node receives two string values on the "in1" and "in2" ports and outputs
/// a boolean result to the "out" port.
pub struct StringEqualNode {
  pub(crate) base: BaseNode,
}

impl StringEqualNode {
  /// Creates a new StringEqualNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::string::StringEqualNode;
  ///
  /// let node = StringEqualNode::new("equal".to_string());
