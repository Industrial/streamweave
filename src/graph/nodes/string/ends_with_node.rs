...
//!
//! A transform node that checks if a string ends with a suffix.
//!
//! ## Ports
//!
//! - **Input**: `"configuration"` - Receives configuration (currently unused, for consistency)
//! - **Input**: `"in"` - Receives string value
//! - **Input**: `"suffix"` - Receives suffix to check for
//! - **Output**: `"out"` - Sends boolean result (true if ends with, false otherwise)
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., type mismatch)
//!
//! ## Behavior
//!
//! The node checks if a string ends with a suffix and returns a boolean. It supports:
...
use crate::graph::nodes::string::common::string_ends_with;
...
/// A node that checks if a string ends with a suffix.
///
...
impl StringEndsWithNode {
  /// Creates a new StringEndsWithNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::string::StringEndsWithNode;
  ///
  /// let node = StringEndsWithNode::new("ends_with".to_string());
  /// // Creates ports: configuration, in, suffix → out, error
  /// ```
...
#[async_trait]
impl Node for StringEndsWithNode {
  fn name(&self) -> &str {
    self.base.name()
  }
      ...
      tokio::spawn(async move {
        ...
        while let Some((port, item)) = merged.next().await {
          ...
          if let (Some(v), Some(suffix)) = (in_buffer.as_ref(), suffix_buffer.as_ref()) {
            match string_ends_with(v, suffix) {
              Ok(result) => {
