//! # And Node
//!
//! A transform node that performs logical AND operation on two boolean inputs.
//!
//! ## Ports
//!
//! - **Input**: `"in1"` - Receives first boolean value
//! - **Input**: `"in2"` - Receives second boolean value
//! - **Output**: `"out"` - Sends the result of (in1 AND in2)
...
/// A node that performs logical AND operation on two boolean inputs.
///
/// The node receives two boolean values on "in1" and "in2" ports and outputs
/// the result of the AND operation to the "out" port.
pub struct AndNode {
  pub(crate) base: BaseNode,
}

impl AndNode {
  /// Creates a new AndNode with the given name.
