...
//!
//! **All data flowing through nodes is automatically wrapped in `Message<T>`.** Nodes handle
//! message wrapping and unwrapping internally, so components work with raw types while the
//! system ensures message IDs and metadata are preserved.
//!
//! ### ProducerNode Message Handling
//!
...
//!
//! // Producer produces raw types (i32)
//! let producer = ProducerNode::from_producer(
//!     "source".to_string(),
//!     ArrayProducer::new([1, 2, 3]),
//! );
//! // Node automatically wraps each item in Message<i32> before sending
//! ```
//!
//! ### TransformerNode Message Handling
//!
//! TransformerNode unwraps `Message<T::Input>`, passes raw types to the transformer, then wraps
//! `Message<T::Output>` before sending:
//!
//! ```rust,no_run
...
//! );
//! // Input: Message<i32> -> unwrap -> i32 -> transform -> i32 -> wrap -> Message<i32>
//! // Message IDs and metadata are preserved through the transformation
//! ```
//!
//! ### ConsumerNode Message Handling
//!
//! ConsumerNode unwraps `Message<C::Input>` before passing raw types to the consumer:
//!
//! ```rust,no_run
//! use crate::graph::nodes::ConsumerNode;
//! use crate::consumers::VecConsumer;
//!
//! // Consumer receives raw types (i32)
//! let consumer = ConsumerNode::from_consumer(
//!     "sink".to_string(),
//!     VecConsumer::<i32>::new(),
//! );
//! // Input: Message<i32> -> unwrap -> i32 -> consumer
//! ```
//!
...
//! to node components.
//!
//! In-process execution mode uses `Arc<Message<T>>` for zero-copy message sharing
//! in fan-out scenarios, further optimizing performance.
//!
//! ## Example
//!
//! ```rust,no_run
...
//! // Output is automatically wrapped in Message<i32>
//! let producer = ProducerNode::from_producer(
//!     "source".to_string(),
//!     ArrayProducer::new([1, 2, 3]),
//! );
...
//! // Input Message<i32> is unwrapped, output is wrapped in Message<i32>
//! let transformer = TransformerNode::from_transformer(
//!     "mapper".to_string(),
//!     MapTransformer::new(|x: i32| x * 2),
//! );
...
//! // Input Message<i32> is unwrapped before passing to consumer
//! let consumer = ConsumerNode::from_consumer(
//!     "sink".to_string(),
//!     VecConsumer::<i32>::new(),
//! );
...
use crate::Consumer;
use crate::ConsumerPorts;
use crate::Input;
use crate::Producer;
use crate::ProducerPorts;
      ...
      while let Some(channel_item) = receiver.recv().await {
        match channel_item {
          ...
          ChannelItem::Arc(arc) => {
            ...
            // First try Message<T::Input>
            let channel_item = crate::graph::channels::ChannelItem::Arc(arc.clone());
            match channel_item.downcast_message_arc::<<T as Input>::Input>() {
              Ok(msg_arc) => {
              ...
              Err(_) => {
                // Fall back to direct T::Input (for backward compatibility)
                match Arc::downcast::<<T as Input>::Input>(arc) {
                  ...
                  Err(_) => {
                    warn!(
                      "Failed to downcast Arc to Message<T::Input> or T::Input, skipping"
                    );
                    continue;
                  }
                }
...
where
  T: crate::graph::zero_copy::ZeroCopyTransformer,
...
where
  T: Transformer + crate::graph::zero_copy::ZeroCopyTransformer + Send,
  T::Input: std::fmt::Debug + Clone + Send + Sync + 'static,
  T::Output: std::fmt::Debug + Clone + Send + Sync + 'static,
{
...
///
/// ```rust
/// use crate::graph::node::ProducerNode;
/// use streamweave_array::ArrayProducer;
///
...
/// );
/// ```
/// A node that wraps a Producer component for graph execution.
///
/// ProducerNode automatically wraps producer output in `Message<T>` before sending to output channels.
/// The producer itself works with raw types, but the node ensures all data flowing through the graph
/// is wrapped in messages with IDs and metadata.
///
/// ## `Message<T>` Handling
///
/// When executed, ProducerNode:
/// 1. Calls the producer's `produce()` method to get a stream of raw types (`P::Output`)
/// 2. Wraps each item in `Message<P::Output>` using `wrap_message()`
/// 3. Sends `Message<P::Output>` to output channels
///
/// This ensures every item has a unique message ID and metadata for tracking through the graph.
///
/// ## Example
///
/// ```rust,no_run
...
/// use streamweave_array::ArrayProducer;
///
/// // Producer produces raw i32 values
/// let producer = ProducerNode::from_producer(
///     "source".to_string(),
///     ArrayProducer::new([1, 2, 3]),
/// );
...
/// A node that wraps a Transformer component for graph execution.
///
/// TransformerNode automatically handles `Message<T>` wrapping and unwrapping. It unwraps
/// `Message<T::Input>` from input channels, passes raw types to the transformer, then wraps
/// `Message<T::Output>` before sending to output channels. Message IDs and metadata are preserved
/// through transformations.
///
/// ## `Message<T>` Handling
///
/// When executed, TransformerNode:
/// 1. Receives `Message<T::Input>` from input channels
/// 2. Unwraps to get raw `T::Input` values
/// 3. Passes raw values to the transformer's `transform()` method
/// 4. Wraps transformer output in `Message<T::Output>` using `wrap_message()`
/// 5. Sends `Message<T::Output>` to output channels
///
/// This ensures message IDs and metadata are preserved through transformations, enabling
...
/// A node that wraps a Consumer component for graph execution.
///
/// ConsumerNode automatically unwraps `Message<C::Input>` from input channels before passing
/// raw types to the consumer. The consumer itself works with raw types, but the node ensures
/// messages are properly unwrapped.
///
/// ## `Message<T>` Handling
///
/// When executed, ConsumerNode:
/// 1. Receives `Message<C::Input>` from input channels
/// 2. Unwraps to get raw `C::Input` values
/// 3. Passes raw values to the consumer's `consume()` method
///
/// This allows consumers to work with raw types while the graph system tracks messages with IDs
/// and metadata throughout the graph.
///
/// # Architecture
///
  ...
  (): ValidateConsumerPorts<C, <C as ConsumerPorts>::DefaultInputPorts>,
{
  ...
  ///
  /// ```rust
  /// use crate::graph::node::ConsumerNode;
  /// use crate::consumers::VecConsumer;
  ///
...
///
/// ```rust
/// use crate::graph::node::OutputRouterNode;
          ...
          tokio::spawn(async move {
            while let Some(item) = output_stream.next().await {
              ...
              if *pause_signal_clone.read().await {
                return Ok::<(), crate::graph::execution::ExecutionError>(());
              }
...
///
/// ```rust
/// use crate::graph::node::InputRouterNode;
