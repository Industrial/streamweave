//! # Array Join Node
//!
//! A transform node that joins array elements into a string using a delimiter.
//!
//! ## Ports
//!
...
//! ## Behavior
//!
//! The node joins array elements into a string with a delimiter and returns the result. It supports:
//! - Joining Vec<Arc<dyn Any + Send + Sync>> arrays
//! - Converting each element to string representation (supports String, i32, i64, u32, u64, f32, f64, bool)
//! - Custom delimiter string
//! - Error handling: Non-array inputs or unsupported element types result in errors sent to the error port

use crate::graph::node::{InputStreams, Node, NodeExecutionError, OutputStreams};
use crate::graph::nodes::array::common::array_join;
...
/// A node that joins array elements into a string using a delimiter.
///
...
impl ArrayJoinNode {
  /// Creates a new ArrayJoinNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::array::ArrayJoinNode;
  ///
  /// let node = ArrayJoinNode::new("join".to_string());
