//! # Array Concat Node
//!
//! A transform node that concatenates two array inputs.
//!
//! ## Ports
//!
//! - **Input**: `"configuration"` - Receives configuration (currently unused, for consistency)
//! - **Input**: `"in1"` - Receives first array value (Vec<Arc<dyn Any + Send + Sync>>)
//! - **Input**: `"in2"` - Receives second array value (Vec<Arc<dyn Any + Send + Sync>>)
//! - **Output**: `"out"` - Sends the concatenated array (in1 + in2)
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., type mismatch)
//!
//! ## Behavior
//!
//! The node concatenates two arrays. It supports:
//! - Array + Array: Direct concatenation (first array, then second array)
...
use crate::graph::nodes::array::common::array_concat;
...
/// A node that concatenates two array inputs.
///
/// The node receives two array values on "in1" and "in2" ports and outputs
...
impl ArrayConcatNode {
  /// Creates a new ArrayConcatNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::array::ArrayConcatNode;
  ///
  /// let node = ArrayConcatNode::new("concat".to_string());
