...
use crate::graph::nodes::aggregation::SumNode;
use std::any::Any;
...
#[tokio::test]
async fn test_sum_node_creation() {
  let node = SumNode::new("test_sum".to_string());
  assert_eq!(node.name(), "test_sum");
...
#[tokio::test]
async fn test_sum_empty_stream() {
  let node = SumNode::new("test_sum".to_string());
...
async fn test_sum_floats() {
  ...
  if let Ok(sum) = results[0].clone().downcast::<f64>() {
    assert!((*sum - 7.5f64).abs() < f64::EPSILON);
  } else {
