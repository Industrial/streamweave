...
fn create_input_streams() -> (
  ...
  inputs.insert(
    ...
    Box::pin(ReceiverStream::new(max_retries_rx)) as crate::graph::node::InputStream,
  );
...
async fn test_retry_node_creation() {
  use crate::graph::nodes::advanced::RetryNode;
...
async fn test_retry_success_first_attempt() {
  use crate::graph::nodes::advanced::{RetryNode, retry_config};
  let node = RetryNode::new("test_retry".to_string(), 100);

  let (_config_tx, data_tx, max_retries_tx, inputs) = create_input_streams();
