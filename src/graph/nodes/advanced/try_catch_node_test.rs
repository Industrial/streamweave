...
fn create_input_streams() -> (
  ...
  inputs.insert(
    "try".to_string(),
    Box::pin(ReceiverStream::new(try_rx)) as crate::graph::node::InputStream,
  );
  inputs.insert(
    "catch".to_string(),
    Box::pin(ReceiverStream::new(catch_rx)) as crate::graph::node::InputStream,
  );
...
async fn test_try_catch_node_creation() {
  use crate::graph::nodes::advanced::TryCatchNode;
...
async fn test_try_catch_success() {
  use crate::graph::nodes::advanced::{TryCatchNode, catch_config, try_config};
  let node = TryCatchNode::new("test_try_catch".to_string());
...
async fn test_try_catch_no_catch_function() {
  use crate::graph::nodes::advanced::{TryCatchNode, try_config};
  let node = TryCatchNode::new("test_try_catch".to_string());
