//! # Object Delete Property Node
//!
//! A transform node that deletes a property from an object (HashMap).
//!
//! ## Ports
//!
...
//! - **Output**: `"out"` - Sends the object with the property deleted
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., type mismatch)
//!
//! ## Behavior
//!
//! The node deletes a property from an object and returns a new object. It supports:
//! - Deleting properties from HashMap<String, Arc<dyn Any + Send + Sync>> objects
//! - Key must be a String
//! - Creates a new object with the property removed (does not modify the original)
//! - If the key doesn't exist, the object is returned unchanged
...
use crate::graph::nodes::object::common::object_delete_property;
...
/// A node that deletes a property from an object.
///
...
impl ObjectDeletePropertyNode {
  /// Creates a new ObjectDeletePropertyNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::object::ObjectDeletePropertyNode;
  ///
  /// let node = ObjectDeletePropertyNode::new("delete_property".to_string());
...
#[async_trait]
impl Node for ObjectDeletePropertyNode {
  fn name(&self) -> &str {
    self.base.name()
  }
      ...
      tokio::spawn(async move {
        ...
        while let Some((port, item)) = merged.next().await {
          ...
          if let (Some(v), Some(key)) = (in_buffer.as_ref(), key_buffer.as_ref()) {
            match object_delete_property(v, key) {
              Ok(result) => {
