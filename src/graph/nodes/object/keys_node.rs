//! # Object Keys Node
//!
//! A transform node that extracts keys from an object (HashMap).
//!
//! ## Ports
//!
//! - **Input**: `"configuration"` - Receives configuration (currently unused, for consistency)
//! - **Input**: `"in"` - Receives object value (HashMap<String, Arc<dyn Any + Send + Sync>>)
//! - **Output**: `"out"` - Sends array of keys (Vec<Arc<dyn Any + Send + Sync>>)
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., type mismatch)
//!
//! ## Behavior
//!
//! The node extracts keys from an object and returns them as an array. It supports:
...
use crate::graph::nodes::object::common::object_keys;
...
/// A node that extracts keys from an object.
///
...
impl ObjectKeysNode {
  /// Creates a new ObjectKeysNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::object::ObjectKeysNode;
  ///
  /// let node = ObjectKeysNode::new("keys".to_string());
