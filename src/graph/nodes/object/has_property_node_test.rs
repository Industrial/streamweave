//! Tests for ObjectHasPropertyNode

use crate::graph::node::{InputStreams, Node};
use crate::graph::nodes::object::ObjectHasPropertyNode;
use std::any::Any;
...
fn create_input_streams() -> (
  ...
  inputs.insert(
    "key".to_string(),
    Box::pin(ReceiverStream::new(key_rx)) as crate::graph::node::InputStream,
  );
...
#[tokio::test]
async fn test_object_has_property_node_creation() {
  let node = ObjectHasPropertyNode::new("test_has_property".to_string());
  assert_eq!(node.name(), "test_has_property");
...
#[tokio::test]
async fn test_object_has_property_true() {
  let node = ObjectHasPropertyNode::new("test_has_property".to_string());
  ...
  // Send values: {"name": "John", "age": 30} with key "name" → true
  ...
  let value = results[0].clone().downcast::<bool>().unwrap();
  assert!(*value);
}

#[tokio::test]
async fn test_object_has_property_false() {
  let node = ObjectHasPropertyNode::new("test_has_property".to_string());
  ...
  // Send values: {"name": "John"} with key "age" → false
  let mut map: HashMap<String, Arc<dyn Any + Send + Sync>> = HashMap::new();
  map.insert(
    "name".to_string(),
    Arc::new("John".to_string()) as Arc<dyn Any + Send + Sync>,
...
#[tokio::test]
async fn test_object_has_property_empty_object() {
  let node = ObjectHasPropertyNode::new("test_has_property".to_string());
  ...
  // Send values: {} with key "name" → false
...
#[tokio::test]
async fn test_object_has_property_non_object_input() {
  let node = ObjectHasPropertyNode::new("test_has_property".to_string());
...
#[tokio::test]
async fn test_object_has_property_invalid_key_type() {
  let node = ObjectHasPropertyNode::new("test_has_property".to_string());
