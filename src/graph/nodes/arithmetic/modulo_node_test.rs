//! Tests for ModuloNode

use crate::graph::node::{InputStreams, Node};
use crate::graph::nodes::arithmetic::ModuloNode;
use std::any::Any;
...
#[tokio::test]
async fn test_modulo_node_creation() {
  let node = ModuloNode::new("test_modulo".to_string());
  assert_eq!(node.name(), "test_modulo");
...
#[tokio::test]
async fn test_modulo_node_i32_zero_remainder() {
  let node = ModuloNode::new("test_modulo".to_string());
  ...
  // Send values: 12 % 4 = 0 (no remainder)
  let _ = in1_tx
    .send(Arc::new(12i32) as Arc<dyn Any + Send + Sync>)
    .await;
  let _ = in2_tx
...
async fn test_modulo_node_i64() {
  ...
  // Send values: 17 % 5 = 2
  let _ = in1_tx
    .send(Arc::new(17i64) as Arc<dyn Any + Send + Sync>)
    .await;
  let _ = in2_tx
...
async fn test_modulo_node_f64() {
  ...
  // Send values: 10.5 % 3.0 = 1.5
  let _ = in1_tx
    .send(Arc::new(10.5f64) as Arc<dyn Any + Send + Sync>)
    .await;
  let _ = in2_tx
  ...
  assert_eq!(results.len(), 1);
  assert!((results[0] - 1.5).abs() < 0.001);
}

#[tokio::test]
async fn test_modulo_node_type_promotion_i32_to_i64() {
  let node = ModuloNode::new("test_modulo".to_string());
...
#[tokio::test]
async fn test_modulo_node_type_promotion_i32_to_f32() {
  let node = ModuloNode::new("test_modulo".to_string());
...
#[tokio::test]
async fn test_modulo_node_division_by_zero_i32() {
  let node = ModuloNode::new("test_modulo".to_string());
...
#[tokio::test]
async fn test_modulo_node_division_by_zero_f64() {
  let node = ModuloNode::new("test_modulo".to_string());
...
#[tokio::test]
async fn test_modulo_node_unsupported_type() {
  let node = ModuloNode::new("test_modulo".to_string());
...
async fn test_modulo_node_u64() {
  ...
  // Send values: 17 % 5 = 2
  let _ = in1_tx
    .send(Arc::new(17u64) as Arc<dyn Any + Send + Sync>)
    .await;
  let _ = in2_tx
...
async fn test_modulo_node_f32() {
  ...
  // Send values: 10.5 % 3.0 = 1.5
  let _ = in1_tx
    .send(Arc::new(10.5f32) as Arc<dyn Any + Send + Sync>)
    .await;
  let _ = in2_tx
    .send(Arc::new(3.0f32) as Arc<dyn Any + Send + Sync>)
    .await;
...
#[tokio::test]
async fn test_modulo_node_negative_result() {
  let node = ModuloNode::new("test_modulo".to_string());
  ...
  // Send values: -10 % 3 = -1 (negative remainder for negative dividend)
  let _ = in1_tx
    .send(Arc::new(-10i32) as Arc<dyn Any + Send + Sync>)
    .await;
  let _ = in2_tx
