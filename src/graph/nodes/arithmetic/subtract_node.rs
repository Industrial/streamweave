//! # Subtract Node
//!
//! A transform node that performs subtraction on two numeric inputs.
//!
//! ## Ports
//!
//! - **Input**: `"configuration"` - Receives configuration (currently unused, for consistency)
//! - **Input**: `"in1"` - Receives first numeric value
//! - **Input**: `"in2"` - Receives second numeric value
//! - **Output**: `"out"` - Sends the result of (in1 - in2)
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., overflow, type mismatch)
//!
//! ## Behavior
//!
//! The node performs subtraction on two numeric values. It supports:
//! - Integer types: i32, i64, u32, u64 (with overflow/underflow checking)
//! - Floating point types: f32, f64
//! - Type promotion: smaller types are promoted to larger types when needed
//! - Overflow/underflow detection: integer operations check for overflow/underflow and emit errors

use crate::graph::node::{InputStreams, Node, NodeExecutionError, OutputStreams};
use crate::graph::nodes::arithmetic::common::subtract_values;
...
/// A node that performs subtraction on two numeric inputs.
///
/// The node receives two numeric values on "in1" and "in2" ports and outputs
/// the result of the subtraction to the "out" port.
pub struct SubtractNode {
  pub(crate) base: BaseNode,
}

impl SubtractNode {
  /// Creates a new SubtractNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::graph::nodes::arithmetic::SubtractNode;
  ///
  /// let node = SubtractNode::new("subtract".to_string());
