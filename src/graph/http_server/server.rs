...
use crate::graph::execution::{ExecutionError, GraphExecutor};
use crate::graph::http_server::nodes::consumer_node::HttpServerConsumerNode;
use crate::graph::http_server::nodes::producer_node::HttpServerProducerNode;
use axum::Router;
use axum::body::Body;
...
/// use streamweave::graph::http_server::{GraphServer, HttpServerProducerNode, HttpServerConsumerNode};
///
...
impl GraphServer {
  ...
  pub async fn from_graph_with_node_names(
    ...
    // Verify node kinds
    if producer_node_trait.node_kind() != crate::graph::traits::NodeKind::Producer {
    ...
    if consumer_node_trait.node_kind() != crate::graph::traits::NodeKind::Consumer {
      return Err(ExecutionError::Other(format!(
  ...
  pub async fn handle(&self, request: Request) -> Response<Body> {
    ...
    request_with_id
      .extensions_mut()
      .insert(crate::graph::http_server::types::RequestIdExtension(
        request_id.clone(),
      ));
