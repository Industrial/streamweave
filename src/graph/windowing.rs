...
mod tests {
  ...
  fn test_graph_window_config_default() {
    ...
    assert!(matches!(
      ...
      crate::window::LateDataPolicy::Drop
    ));
  }
  ...
  fn test_graph_window_config_with_late_data_policy() {
    let config = GraphWindowConfig::count_tumbling(10).with_late_data_policy(
      crate::window::LateDataPolicy::AllowLateness(Duration::from_secs(10)),
    );
    assert!(matches!(
      config.late_data_policy,
      crate::window::LateDataPolicy::AllowLateness(_)
    ));

    let config = GraphWindowConfig::count_tumbling(10)
      .with_late_data_policy(crate::window::LateDataPolicy::SideOutput);
    assert!(matches!(
      config.late_data_policy,
      crate::window::LateDataPolicy::SideOutput
    ));
  }
  ...
  fn test_window_config_method_chaining() {
    let config = GraphWindowConfig::count_tumbling(10)
      ...
      .with_late_data_policy(crate::window::LateDataPolicy::AllowLateness(
