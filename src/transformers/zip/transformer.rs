...
impl<T: std::fmt::Debug + Clone + Send + Sync + 'static> Transformer for ZipTransformer<T> {
  ...
  fn set_config_impl(&mut self, config: crate::transformer::TransformerConfig<Vec<T>>) {
    self.config = config;
  }

  fn get_config_impl(&self) -> &crate::transformer::TransformerConfig<Vec<T>> {
    &self.config
  }

  fn get_config_mut_impl(&mut self) -> &mut crate::transformer::TransformerConfig<Vec<T>> {
    &mut self.config
  }
