...
impl CorsTransformer {
    ...
    fn is_header_allowed(&self, header: &str) -> bool {
        ...
        self.config.allowed_headers.iter().any(|h| h.to_lowercase() == header_lower)
    }

    #[allow(dead_code)]
    fn handle_preflight_request(&self, request: &StreamWeaveHttpRequestChunk) -> StreamWeaveHttpResponse {
        ...
        // Add allowed methods
        let methods_str = self.config.allowed_methods
            .iter()
            .map(|m| m.as_str())
            .collect::<Vec<_>>()
            .join(", ");
...
mod tests {
    ...
    fn test_origin_allowed() {
        ...
        config.allowed_origins = vec!["https://example.com".to_string(), "https://test.com".to_string()];
    ...
    fn test_cors_response_transformer() {
        ...
        let mut response = StreamWeaveHttpResponse::new(
            StatusCode::OK,
            HeaderMap::new(),
