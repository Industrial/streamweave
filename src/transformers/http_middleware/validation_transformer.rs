  ...
  async fn validate_request(&self, request: &StreamWeaveHttpRequestChunk) -> ValidationResult {
    ...
    for rule in self.rules.iter() {
      match rule {
              ...
              if !content_type_str.starts_with(expected_type) {
                all_errors.push(CustomValidationError::new(
                  ...
                  &format!(
                    ...
                    expected_type, content_type_str
                  ),
        ...
        ValidationRule::Custom(validator) => match validator.validate(request).await {
          ValidationResult::Valid => {}
          ValidationResult::Invalid { errors } => {
            all_errors.extend(errors);
          }
...
mod tests {
  ...
  fn test_validation_error_creation() {
    let error = CustomValidationError::new("field", "Error message", "error_code");
  ...
  async fn test_request_validation_transformer_required_headers() {
    let transformer =
      RequestValidationTransformer::new().with_rule(ValidationRule::RequiredHeaders(vec![
  ...
  async fn test_request_validation_transformer_max_body_size() {
    let transformer =
      RequestValidationTransformer::new().with_rule(ValidationRule::MaxBodySize(100));
  ...
  fn test_validation_transformer_creation() {
    let transformer = RequestValidationTransformer::new()
      .with_rule(ValidationRule::RequiredHeaders(vec![
        "authorization".to_string(),
      ]))
