...
pub trait TokenValidator: Send + Sync {
    async fn validate(&self, token: &str) -> Result<UserInfo, Box<dyn std::error::Error + Send + Sync>>;
}
...
impl TokenValidator for JwtTokenValidator {
    async fn validate(&self, _token: &str) -> Result<UserInfo, Box<dyn std::error::Error + Send + Sync>> {
...
impl AuthTransformer {
    ...
    pub fn with_strategy(mut self, strategy: Box<dyn AuthStrategy>) -> Self {
        Arc::get_mut(&mut self.auth_strategies).unwrap().push(strategy);
        self
    }
    ...
    #[allow(dead_code)]
    async fn authenticate_request(&self, request: &StreamWeaveHttpRequestChunk) -> Result<UserInfo, AuthError> {
            ...
            match strategy.authenticate(request).await {
                AuthResult::Authenticated { user_id, roles } => {
                    ...
                    if !self.required_roles.is_empty() {
                        let has_required_role = self.required_roles.iter()
                ...
                AuthResult::Unauthenticated { reason } => {
                    return Err(AuthError::Unauthenticated { reason });
                }
                AuthResult::Error { message } => {
                    return Err(AuthError::ValidationError { message });
                }
            }
        }
...
mod tests {
    ...
    async fn test_api_key_auth_success() {
        ...
        valid_keys.insert("valid_key".to_string(), UserInfo {
    ...
    async fn test_auth_transformer_with_roles() {
        ...
        let request = create_test_request(headers);
        let mut valid_keys = HashMap::new();
        valid_keys.insert("admin_key".to_string(), UserInfo {
    ...
    async fn test_auth_transformer_insufficient_permissions() {
        ...
        let request = create_test_request(headers);
        let mut valid_keys = HashMap::new();
        valid_keys.insert("user_key".to_string(), UserInfo {
