...
pub trait ResponseBodyTransformer: Send + Sync {
    async fn transform(&self, body: &[u8], content_type: Option<&str>) -> Result<Vec<u8>, TransformError>;
}
...
impl ResponseBodyTransformer for JsonResponseTransformer {
    async fn transform(&self, body: &[u8], _content_type: Option<&str>) -> Result<Vec<u8>, TransformError> {
        // Parse JSON
        let mut json_value: serde_json::Value = serde_json::from_slice(body)
            .map_err(|e| TransformError::JsonParseError { message: e.to_string() })?;
        ...
        result
            .map(|s| s.into_bytes())
            .map_err(|e| TransformError::JsonSerializeError { message: e.to_string() })
    }
...
impl ResponseTransformTransformer {
    ...
    #[allow(dead_code)]
    async fn transform_response(&self, mut response: StreamWeaveHttpResponse) -> Result<StreamWeaveHttpResponse, TransformError> {
        for rule in self.rules.iter() {
            match rule {
                ...
                ResponseTransformRule::AddCorsHeaders { allowed_origins, allowed_methods, allowed_headers } => {
                    self.add_cors_headers(&mut response.headers, &allowed_origins, &allowed_methods, &allowed_headers);
                }
                ...
                ResponseTransformRule::AddCacheHeaders { max_age, cache_control } => {
    ...
    #[allow(dead_code)]
    fn add_cors_headers(&self, headers: &mut HeaderMap, allowed_origins: &[String], allowed_methods: &[String], allowed_headers: &[String]) {
    ...
    fn add_cache_headers(&self, headers: &mut HeaderMap, max_age: u64, cache_control: &str) {
        headers.insert("cache-control", HeaderValue::from_str(cache_control).unwrap_or(HeaderValue::from_static("public")));
        headers.insert("expires", HeaderValue::from_str(&format!("{}", max_age)).unwrap_or(HeaderValue::from_static("0")));
    }
}
...
mod tests {
    ...
    async fn test_json_response_transformer_valid_json() {
        ...
        let result = transformer.transform(json_data.as_bytes(), Some("application/json")).await;
        assert!(result.is_ok());
    ...
    async fn test_json_response_transformer_invalid_json() {
        ...
        let result = transformer.transform(invalid_json.as_bytes(), Some("application/json")).await;
    ...
    async fn test_html_response_transformer_add_doctype() {
        ...
        let result = transformer.transform(html_data.as_bytes(), Some("text/html")).await;
        assert!(result.is_ok());
    ...
    fn test_response_transform_transformer_add_cache_headers() {
        ...
        headers.insert("cache-control", HeaderValue::from_str("public, max-age=3600").unwrap_or(HeaderValue::from_static("public")));
        headers.insert("expires", HeaderValue::from_str("3600").unwrap_or(HeaderValue::from_static("0")));
