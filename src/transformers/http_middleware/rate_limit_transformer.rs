...
pub enum RateLimitStrategy {
    ...
    FixedWindow { window_seconds: u64, max_requests: u32 },
    /// Sliding window: rolling window of N seconds
    SlidingWindow { window_seconds: u64, max_requests: u32 },
    /// Token bucket: refill tokens at a rate
    TokenBucket { capacity: u32, refill_rate: f64, refill_period: Duration },
}
...
impl RateLimiter {
    ...
    pub async fn is_allowed(&self, request: &StreamWeaveHttpRequestChunk) -> RateLimitResult {
        ...
        match &self.strategy {
            RateLimitStrategy::FixedWindow { window_seconds, max_requests } => {
                self.check_fixed_window(&key, *window_seconds, *max_requests, now).await
            }
            RateLimitStrategy::SlidingWindow { window_seconds, max_requests } => {
                self.check_sliding_window(&key, *window_seconds, *max_requests, now).await
            }
            RateLimitStrategy::TokenBucket { capacity, refill_rate, refill_period } => {
                self.check_token_bucket(&key, *capacity, *refill_rate, *refill_period, now).await
            }
        }
    }

    async fn check_fixed_window(&self, key: &str, window_seconds: u64, max_requests: u32, now: Instant) -> RateLimitResult {
    ...
    async fn check_sliding_window(&self, key: &str, window_seconds: u64, max_requests: u32, now: Instant) -> RateLimitResult {
        let mut entries = self.entries.write().await;
        let entry = entries.entry(key.to_string()).or_insert_with(RateLimitEntry::new);
    ...
    async fn check_token_bucket(&self, key: &str, capacity: u32, refill_rate: f64, refill_period: Duration, now: Instant) -> RateLimitResult {
        let mut buckets = self.token_buckets.write().await;
        let bucket = buckets.entry(key.to_string()).or_insert_with(|| {
            TokenBucket::new(capacity, refill_rate, refill_period)
        });
    ...
    fn create_rate_limit_response(&self, result: &RateLimitResult) -> StreamWeaveHttpResponse {
        ...
        match result {
            RateLimitResult::Allowed { remaining, reset_time } => {
                ...
                headers.insert("x-ratelimit-reset", reset_time.duration_since(Instant::now()).as_secs().to_string().parse().unwrap());
            ...
            RateLimitResult::RateLimited { retry_after } => {
                headers.insert("retry-after", retry_after.as_secs().to_string().parse().unwrap());
...
mod tests {
    ...
    fn test_custom_key_extractor() {
        let extractor = CustomKeyExtractor::new(|req| {
