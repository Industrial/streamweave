...
impl CompressionTransformer {
    ...
    fn should_compress(&self, response: &StreamWeaveHttpResponse) -> bool {
        ...
        if !self.config.content_types.is_empty() {
            let should_compress_type = self.config.content_types.iter()
    ...
    #[allow(dead_code)]
    async fn compress_body(&self, body: &[u8], algorithm: CompressionAlgorithm) -> Result<Vec<u8>, CompressionError> {
        match algorithm {
            CompressionAlgorithm::Gzip => {
                self.compress_gzip(body).await
            }
            CompressionAlgorithm::Deflate => {
                self.compress_deflate(body).await
            }
            CompressionAlgorithm::Brotli => {
                self.compress_brotli(body).await
            }
            CompressionAlgorithm::Zstd => {
                self.compress_zstd(body).await
            }
        }
    }
    ...
    async fn compress_gzip(&self, body: &[u8]) -> Result<Vec<u8>, CompressionError> {
        ...
        encoder.write_all(body)
            .map_err(|e| CompressionError::CompressionFailed { message: e.to_string() })?;
        encoder.finish()
            .map_err(|e| CompressionError::CompressionFailed { message: e.to_string() })
    }
    ...
    async fn compress_brotli(&self, _body: &[u8]) -> Result<Vec<u8>, CompressionError> {
        ...
        Err(CompressionError::UnsupportedAlgorithm { algorithm: "brotli".to_string() })
    }
    ...
    async fn compress_zstd(&self, _body: &[u8]) -> Result<Vec<u8>, CompressionError> {
        ...
        Err(CompressionError::UnsupportedAlgorithm { algorithm: "zstd".to_string() })
    }

    // Static methods for use in async streams
    fn should_compress_static(config: &CompressionConfig, response: &StreamWeaveHttpResponse) -> bool {
        // Check if response is already compressed
        if response.headers.get("content-encoding").is_some() {
            return false;
        }
        ...
        if !config.content_types.is_empty() {
            let should_compress_type = config.content_types.iter()
                .any(|ct| content_type.starts_with(ct));
            if !should_compress_type {
                return false;
            }
    ...
    fn get_best_algorithm_static(config: &CompressionConfig, accept_encoding: &str) -> Option<CompressionAlgorithm> {
        let accepted: HashSet<String> = accept_encoding
            .split(',')
            .map(|s| s.trim().to_lowercase())
            .collect();
    ...
    async fn compress_body_static(config: &CompressionConfig, body: &[u8], algorithm: CompressionAlgorithm) -> Result<Vec<u8>, CompressionError> {
        match algorithm {
            CompressionAlgorithm::Gzip => {
                Self::compress_gzip_static(config, body).await
            }
            CompressionAlgorithm::Deflate => {
                Self::compress_deflate_static(config, body).await
            }
            CompressionAlgorithm::Brotli => {
                Self::compress_brotli_static(config, body).await
            }
            CompressionAlgorithm::Zstd => {
                Self::compress_zstd_static(config, body).await
            }
        }
    }

    async fn compress_gzip_static(config: &CompressionConfig, body: &[u8]) -> Result<Vec<u8>, CompressionError> {
        use flate2::write::GzEncoder;
        use flate2::Compression;
        use std::io::Write;
    ...
    async fn compress_deflate_static(config: &CompressionConfig, body: &[u8]) -> Result<Vec<u8>, CompressionError> {
        use flate2::write::DeflateEncoder;
        use flate2::Compression;
        use std::io::Write;
    ...
    async fn compress_brotli_static(_config: &CompressionConfig, _body: &[u8]) -> Result<Vec<u8>, CompressionError> {
        Err(CompressionError::UnsupportedAlgorithm { algorithm: "brotli".to_string() })
    }

    async fn compress_zstd_static(_config: &CompressionConfig, _body: &[u8]) -> Result<Vec<u8>, CompressionError> {
        Err(CompressionError::UnsupportedAlgorithm { algorithm: "zstd".to_string() })
    }
}
...
impl DecompressionTransformer {
    ...
    #[allow(dead_code)]
    async fn decompress_body(&self, body: &[u8], encoding: &str) -> Result<Vec<u8>, CompressionError> {
        match encoding.to_lowercase().as_str() {
            ...
            _ => Err(CompressionError::UnsupportedAlgorithm { algorithm: encoding.to_string() }),
        }
    }
    ...
    // Static methods for use in async streams
    fn should_decompress_static(_config: &CompressionConfig, request: &StreamWeaveHttpRequestChunk) -> bool {
        request.headers.get("content-encoding").is_some()
    }

    async fn decompress_body_static(_config: &CompressionConfig, body: &[u8], encoding: &str) -> Result<Vec<u8>, CompressionError> {
        match encoding.to_lowercase().as_str() {
...
mod tests {
    ...
    use http::{Method, Uri, Version, StatusCode, HeaderMap};
    ...
    fn test_compression_algorithm_from_str() {
        ...
        assert_eq!(CompressionAlgorithm::from_str("brotli"), Some(CompressionAlgorithm::Brotli));
    ...
    async fn test_gzip_decompression() {
        ...
        let compressed = CompressionTransformer::compress_gzip_static(&CompressionConfig::default(), test_data).await.unwrap();
