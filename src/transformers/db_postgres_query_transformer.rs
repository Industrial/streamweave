...
/// A transformer that executes PostgreSQL queries from input items.
///
/// Input can be:
...
/// // Output: [DatabaseRow, ...]
/// ```
pub struct DbPostgresQueryTransformer {
...
impl DbPostgresQueryTransformer {
  /// Creates a new `DbPostgresQueryTransformer` with the given database configuration.
  ///
  ...
  pub fn new(mut db_config: DatabaseProducerConfig) -> Self {
    // Ensure database type is PostgreSQL
...
impl Clone for DbPostgresQueryTransformer {
  fn clone(&self) -> Self {
    Self {
...
impl Input for DbPostgresQueryTransformer {
  type Input = String;
  type InputStream = Pin<Box<dyn Stream<Item = String> + Send>>;
}

impl Output for DbPostgresQueryTransformer {
  type Output = DatabaseRow;
  type OutputStream = Pin<Box<dyn Stream<Item = DatabaseRow> + Send>>;
}

#[async_trait]
impl Transformer for DbPostgresQueryTransformer {
  ...
  async fn transform(&mut self, input: Self::InputStream) -> Self::OutputStream {
    ...
    let component_name = self
      ...
      .name
      .clone()
      .unwrap_or_else(|| "db_postgres_query_transformer".to_string());
          ...
          Err(e) => {
            ...
            let stream_error = StreamError::new(
              ...
              ErrorContext {
                ...
                component_type: std::any::type_name::<DbPostgresQueryTransformer>().to_string(),
              },
              ComponentInfo {
                name: component_name.clone(),
                type_name: std::any::type_name::<DbPostgresQueryTransformer>().to_string(),
              },
            );
...
async fn execute_query(
  ...
  for param in params {
    query_builder = bind_parameter_postgres(query_builder, param)?;
  }
  ...
  Ok(
    rows
      .into_iter()
      .map(|row| convert_postgres_row_to_database_row(&row))
      .collect(),
  )
}
...
fn convert_postgres_row_to_database_row(row: &sqlx::postgres::PgRow) -> DatabaseRow {
