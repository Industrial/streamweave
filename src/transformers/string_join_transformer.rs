...
/// A transformer that joins an array of strings into a single string.
///
...
/// // Output: ["a,b,c", "x,y"]
/// ```
pub struct StringJoinTransformer {
...
impl StringJoinTransformer {
  /// Creates a new `StringJoinTransformer` with the specified delimiter.
  ///
  /// # Arguments
  ///
...
impl Clone for StringJoinTransformer {
  fn clone(&self) -> Self {
    Self {
...
impl Input for StringJoinTransformer {
  type Input = Vec<String>;
  type InputStream = Pin<Box<dyn Stream<Item = Vec<String>> + Send>>;
}

impl Output for StringJoinTransformer {
  type Output = String;
  type OutputStream = Pin<Box<dyn Stream<Item = String> + Send>>;
}

#[async_trait]
impl Transformer for StringJoinTransformer {
  type InputPorts = (Vec<String>,);
  type OutputPorts = (String,);

  async fn transform(&mut self, input: Self::InputStream) -> Self::OutputStream {
    let delimiter = self.delimiter.clone();
    Box::pin(input.map(move |vec| vec.join(&delimiter)))
  }
  ...
  fn component_info(&self) -> ComponentInfo {
    ComponentInfo {
      name: self
        ...
        .name
        .clone()
        .unwrap_or_else(|| "string_join_transformer".to_string()),
