...
impl<T> crate::traits::error::Error for DistinctTransformer<T>
where
  T: Send + 'static,
{
...
mod tests {
  ...
  async fn test_distinct_basic() {
    ...
    let input = stream::iter(vec![1, 2, 2, 3, 3, 3].into_iter().map(Ok));
  ...
  #[tokio::test]
  async fn test_distinct_with_error() {
    let mut transformer = DistinctTransformer::new();
