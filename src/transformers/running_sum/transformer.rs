...
use crate::transformers::running_sum::running_sum_transformer::RunningSumTransformer;
...
impl<T: Clone + Send + Sync> StateStore<T> for SharedStateStore<T> {
  fn get(&self) -> crate::stateful_transformer::StateResult<Option<T>> {
    self.0.get()
  }

  fn set(&self, state: T) -> crate::stateful_transformer::StateResult<()> {
    self.0.set(state)
  }
  ...
  ) -> crate::stateful_transformer::StateResult<T> {
