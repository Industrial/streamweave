...
/// A transformer that writes DatabaseRow data to SQLite while passing data through.
///
/// Each input DatabaseRow is written to the SQLite table (with batching for performance),
...
/// // Writes to SQLite and outputs: [DatabaseRow, ...]
/// ```
pub struct DbSqliteWriteTransformer {
...
impl DbSqliteWriteTransformer {
  /// Creates a new `DbSqliteWriteTransformer` with the given database configuration.
  ///
  /// # Arguments
  ///
...
impl Clone for DbSqliteWriteTransformer {
  fn clone(&self) -> Self {
    Self {
...
impl Transformer for DbSqliteWriteTransformer {
  ...
  async fn transform(&mut self, input: Self::InputStream) -> Self::OutputStream {
    let component_name = self
      ...
      .name
      .clone()
      .unwrap_or_else(|| "db_sqlite_write_transformer".to_string());
              ...
              Err(e) => {
                let stream_error = StreamError::new(
                  ...
                  ErrorContext {
                    ...
                    component_type: std::any::type_name::<DbSqliteWriteTransformer>().to_string(),
                  },
                  ComponentInfo {
                    name: component_name.clone(),
                    type_name: std::any::type_name::<DbSqliteWriteTransformer>().to_string(),
                  },
                );
