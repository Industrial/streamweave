...
/// A transformer that writes DatabaseRow data to MySQL while passing data through.
///
/// Each input DatabaseRow is written to the MySQL table (with batching for performance),
...
/// // Writes to MySQL and outputs: [DatabaseRow, ...]
/// ```
pub struct DbMysqlWriteTransformer {
...
impl DbMysqlWriteTransformer {
  /// Creates a new `DbMysqlWriteTransformer` with the given database configuration.
  ///
  /// # Arguments
  ///
...
impl Clone for DbMysqlWriteTransformer {
  fn clone(&self) -> Self {
    Self {
...
impl Transformer for DbMysqlWriteTransformer {
  ...
  async fn transform(&mut self, input: Self::InputStream) -> Self::OutputStream {
    let component_name = self
      ...
      .name
      .clone()
      .unwrap_or_else(|| "db_mysql_write_transformer".to_string());
              ...
              Err(e) => {
                let stream_error = StreamError::new(
                  ...
                  ErrorContext {
                    ...
                    component_type: std::any::type_name::<DbMysqlWriteTransformer>().to_string(),
                  },
                  ComponentInfo {
                    name: component_name.clone(),
                    type_name: std::any::type_name::<DbMysqlWriteTransformer>().to_string(),
                  },
                );
