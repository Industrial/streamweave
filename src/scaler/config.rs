...
impl ScalerConfig {
  ...
  pub fn validate(&self) -> Result<(), String> {
    ...
    if (self.scale_up_threshold_items_per_sec.is_some()
      ...
      && self.scale_up_stabilization_secs == 0
    {
      return Err(
        "scale_up_stabilization_secs must be > 0 when scale-up thresholds are set".to_string(),
      );
    }
    if (self.scale_down_threshold_items_per_sec.is_some()
      || self.scale_down_threshold_backlog.is_some())
      && self.scale_down_stabilization_secs == 0
    {
      return Err(
        "scale_down_stabilization_secs must be > 0 when scale-down thresholds are set".to_string(),
      );
    }
