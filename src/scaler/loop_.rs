...
use super::policy::{PolicyInputs, ScalerDecision, decide};
...
pub fn run_one_tick(
  ...
  let scale_up_condition = (config
    .scale_up_threshold_items_per_sec
    .is_some_and(|t| metrics.items_per_sec >= t))
    || (config
      .scale_up_threshold_backlog
      .is_some_and(|t| metrics.backlog >= t));

  let scale_down_condition = (config
    .scale_down_threshold_items_per_sec
    .is_none_or(|t| metrics.items_per_sec <= t))
    && (config
      .scale_down_threshold_backlog
      .is_none_or(|t| metrics.backlog <= t));
  ...
  let scale_up_stabilized = scale_up_satisfied_since
    .is_some_and(|t| now.duration_since(t).as_secs() >= config.scale_up_stabilization_secs);
  let scale_down_stabilized = scale_down_satisfied_since
    .is_some_and(|t| now.duration_since(t).as_secs() >= config.scale_down_stabilization_secs);
...
#[cfg(test)]
mod tests {
  use super::{ScalerState, TickMetrics, run_one_tick};
  use crate::rebalance::{InMemoryCoordinator, RebalanceCoordinator};
  use crate::scaler::{ScaleReason, ScalerConfig, record_scale_decision};
  use std::time::Instant;
  ...
  async fn integration_tick_then_scale_to_updates_coordinator() {
    ...
    let (decision, new_state) =
      run_one_tick(&config, &state, assignment.total_shards, metrics, now);
    state = new_state;
    ...
    let (decision2, _new_state2) =
      run_one_tick(&config, &state, assignment2.total_shards, metrics, now2);
