...
//!
//! ```rust,no_run
//! use streamweave_ml_transformers::{InferenceBackend, OnnxBackend};
//!
...
/// let mut gpu_backend = OnnxBackendGeneric::with_provider(ExecutionProvider::CUDA(Default::default()))?;
/// gpu_backend.load_from_path("model.onnx").await?;
/// # Ok(())
/// # }
/// ```
pub struct OnnxBackendGeneric<T: ExecutionProvider + Clone> {
...
/// Type alias for OnnxBackend with default CPU execution provider.
pub type OnnxBackend = OnnxBackendGeneric<ort::execution_providers::CPUExecutionProvider>;

impl OnnxBackend {
  /// Creates a new ONNX backend with default CPU execution provider.
  ///
  /// # Returns
  ///
  /// Returns a new `OnnxBackend` instance configured with CPU execution provider.
  ///
  /// # Errors
  ///
  ...
  pub fn new() -> Result<Self, OnnxError> {
    Self::with_provider(ort::execution_providers::CPUExecutionProvider::default())
  }
}

impl<T> OnnxBackendGeneric<T>
where
...
impl<T: ExecutionProvider + Clone> InferenceBackend for OnnxBackendGeneric<T> {
  type Input = Vec<f32>;
  type Output = Vec<f32>;
  ...
  async fn infer(&self, input: Self::Input) -> Result<Self::Output, Self::Error> {
    ...
    let input_name = {
      let session_read_guard = self.session.read().await;
      let session_ref = session_read_guard
        .as_ref()
        .ok_or_else(|| OnnxError::new("Model not loaded"))?;
      session_ref
        .inputs()
