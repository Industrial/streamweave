...
use crate::rebalance::{
  InMemoryCoordinator, RebalanceCoordinator, ShardAssignment, compute_migration_plan,
};
...
async fn test_compute_migration_plan_no_change() {
  let keys: Vec<PartitionKey> = ["a", "b", "c"]
    .iter()
...
async fn test_compute_migration_plan_add_worker() {
  ...
  let keys: Vec<PartitionKey> = (0..20).map(|i| PartitionKey::from(i.to_string())).collect();
