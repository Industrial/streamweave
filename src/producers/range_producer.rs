...
mod tests {
  ...
  #[tokio::test]
  async fn test_range_producer_ascending() {
    let mut producer = RangeProducer::new(1, 6, 1);
  ...
  #[tokio::test]
  async fn test_range_producer_with_step() {
  ...
  #[tokio::test]
  async fn test_range_producer_descending() {
    let mut producer = RangeProducer::new(10, 5, -1);
    let mut stream = producer.produce();
    ...
    assert_eq!(results, vec![10, 9, 8, 7, 6]);
  }

  #[tokio::test]
  async fn test_range_producer_empty() {
    let mut producer = RangeProducer::new(5, 5, 1);
    let mut stream = producer.produce();
  ...
  #[tokio::test]
  async fn test_range_producer_single_item() {
    let mut producer = RangeProducer::new(42, 43, 1);
    let mut stream = producer.produce();
  ...
  #[tokio::test]
  async fn test_range_producer_with_name() {
    let producer = RangeProducer::new(1, 3, 1).with_name("test-range".to_string());
    assert_eq!(producer.config.name, Some("test-range".to_string()));
  }

  #[tokio::test]
  async fn test_range_producer_with_error_strategy() {
    let producer = RangeProducer::new(1, 3, 1).with_error_strategy(ErrorStrategy::Skip);
    assert!(matches!(
