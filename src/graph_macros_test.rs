...
async fn test_graph_macro_simple_linear_pipeline() {
    ...
    // Verify edges exist
    assert!(graph.find_edge_by_nodes_and_ports("producer", "out", "transform", "in").is_some());
    assert!(graph.find_edge_by_nodes_and_ports("transform", "out", "sink", "in").is_some());
...
fn test_graph_macro_fan_out_rejected() {
    ...
    let builder = GraphBuilder::new("test")
        .add_node("source", Box::new(MockProducerNode::new("source".to_string(), vec![1, 2, 3])))
        .add_node("filter1", Box::new(MockTransformNode::new("filter1".to_string())))
        .add_node("filter2", Box::new(MockTransformNode::new("filter2".to_string())))
...
async fn test_graph_macro_graph_io_with_values() {
    ...
    graph.expose_output_port("transform", "out", "result").unwrap();
...
async fn test_graph_macro_combined_patterns() {
    ...
    graph.expose_input_port("transform", "in", "config").unwrap();
    graph.expose_output_port("transform", "out", "output").unwrap();
