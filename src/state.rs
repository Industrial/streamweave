...
pub trait StatefulNodeDriver: Send + Sync {
  ...
  fn get_value(&self, key: &Self::Key) -> Result<Option<(Self::Value, LogicalTime)>, StateError>;
  ...
  Ver: Ord + Clone + Send + serde::Serialize + for<'de> serde::Deserialize<'de>,
{
  ...
  fn put(
    ...
    let should_apply = map.get(&key).is_none_or(|(_, v)| version > *v);
    if should_apply {
...
mod tests {
  ...
  fn hash_map_state_backend_snapshot_restore() {
    let backend: HashMapStateBackend<String, u64, LogicalTime> = HashMapStateBackend::new();
    backend
      .put("a".to_string(), 1u64, LogicalTime::new(1))
      .unwrap();
    backend
      .put("b".to_string(), 2u64, LogicalTime::new(2))
      .unwrap();
    ...
    assert_eq!(
      restored.get(&"a".to_string()).unwrap(),
      Some((1u64, LogicalTime::new(1)))
    );
    assert_eq!(
      restored.get(&"b".to_string()).unwrap(),
      Some((2u64, LogicalTime::new(2)))
    );
  }
  ...
  fn hash_map_state_backend_snapshot_for_keys_restore_keys() {
    ...
    backend
      ...
      .unwrap();
    backend
      .put("c".to_string(), 3u64, LogicalTime::new(3))
      .unwrap();
    ...
    target
      .put("x".to_string(), 99u64, LogicalTime::new(0))
      .unwrap();
    target.restore_keys(&data).unwrap();

    assert_eq!(
      target.get(&"a".to_string()).unwrap(),
      Some((1u64, LogicalTime::new(1)))
    );
    assert_eq!(
      target.get(&"c".to_string()).unwrap(),
      Some((3u64, LogicalTime::new(3)))
    );
    assert_eq!(
      target.get(&"x".to_string()).unwrap(),
      Some((99u64, LogicalTime::new(0)))
    );
  ...
  fn stateful_node_driver_apply_and_get() {
    ...
    backend
      .apply_update("x".to_string(), 42u64, LogicalTime::new(3))
      .unwrap();
    assert_eq!(
    ...
    backend
      .apply_update("x".to_string(), 99u64, LogicalTime::new(3))
      .unwrap();
    assert_eq!(
      backend.get_value(&"x".to_string()).unwrap(),
      Some((42u64, LogicalTime::new(3)))
