...
function getNodeColor(kind) {
    switch (kind) {
        case 'producer': return 'lightblue';
        case 'transformer': return 'lightgreen';
        case 'consumer': return 'lightcoral';
...
function renderGraph() {
    if (!dagData) {
        svg.innerHTML = '<text x="50%" y="50%" text-anchor="middle" fill="#888">No DAG data available</text>';
        return;
    }
    ...
    dagData.nodes.forEach((node, index) => {
        ...
        text.textContent = node.metadata.component_type.split('::').pop() || node.id;
        nodeGroup.appendChild(text);
        
        // Add throughput text (will be updated by real-time metrics)
        const throughputText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
...
function startRealTimeUpdates() {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const realtimeEnabled = urlParams.get('realtime') === 'true' || window.embeddedRealtimeEnabled;
...
function updateMetricsFromData(metricsData) {
    ...
    nodeMetrics.forEach((metrics, nodeId) => {
        const throughputText = svg.querySelector(`[data-node-throughput="${nodeId}"]`);
        if (throughputText) {
...
function animateDataFlow() {
    ...
    dagData.edges.forEach(edge => {
        const marker = svg.querySelector(`[data-flow-marker="${edge.from}-${edge.to}"]`);
...
function updateBreakpointList() {
    ...
    if (breakpoints.size === 0) {
        breakpointList.innerHTML = '<p style="color: #888; font-size: 0.9rem;">No breakpoints set</p>';
        return;
    }
...
function updateDebugControls() {
    const resumeBtn = document.querySelector('#debug-controls button[onclick="debugResume()"]');
    const stepBtn = document.querySelector('#debug-controls button[onclick="debugStep()"]');
    const pauseBtn = document.querySelector('#debug-controls button[onclick="debugPause()"]');
...
function updateDebugSnapshots(snapshots) {
    ...
    if (!snapshots || snapshots.length === 0) {
        snapshotList.innerHTML = '<p style="color: #888; font-size: 0.9rem;">No snapshots yet</p>';
        return;
    }
