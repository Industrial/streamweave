//! SQLite database consumer for writing stream data to SQLite tables.
//!
//! This module provides [`DbSqliteConsumer`], a consumer that writes [`DatabaseRow`]
//! items to SQLite database tables. It supports batch insertion for performance,
//! connection pooling, and transaction management through sqlx.
//!
//! # Overview
//!
//! [`DbSqliteConsumer`] is useful for persisting stream data to SQLite databases.
...
//! # Core Types
//!
//! - **[`DbSqliteConsumer`]**: Consumer that writes rows to SQLite tables
//!
//! # Quick Start
//!
//! ## Basic Usage
...
//!
//! // Create a consumer
//! let mut consumer = DbSqliteConsumer::new(db_config);
//!
...
//! ]);
//!
//! // Consume the stream (rows inserted into SQLite)
...
//!
//! let db_config = DatabaseConsumerConfig { /* ... */ };
//! let consumer = DbSqliteConsumer::new(db_config)
...
//!
//! [`DbSqliteConsumer`] implements the [`Consumer`] trait and can be used in any
...
impl DbSqliteConsumer {
  /// Creates a new SQLite consumer with the given configuration.
  #[must_use]
...
impl Clone for DbSqliteConsumer {
  fn clone(&self) -> Self {
    Self {
...
) -> Result<sqlx::SqlitePool, Box<dyn std::error::Error + Send + Sync + 'static>> {
...
> {
  let bound_query = match value {
    ...
    Some(serde_json::Value::Bool(b)) => query.bind(if *b { 1i64 } else { 0i64 }), // SQLite uses integer for boolean
