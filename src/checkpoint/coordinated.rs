...
/// Pending checkpoint state: map from checkpoint id to (notify, count, aborted flag).
type PendingCheckpointMap = std::collections::HashMap<
  CheckpointId,
  (
...
impl InMemoryCheckpointCoordinator {
  ...
  pub fn new(total_shards: u32) -> Self {
    Self {
      ...
      pending: std::sync::RwLock::new(PendingCheckpointMap::new()),
    }
  }
}
    ...
    let keys_to_import: Vec<PartitionKey> = match keys {
      Some(k) => {
        let failed_owns = |s: &str| {
          crate::rebalance::ShardAssignment::new(failed_shard_id, old_total_shards).owns_key(s)
        };
