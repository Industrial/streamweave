...
pub struct NodeRegistry {
    ...
    constructors: HashMap<
        String,
        Box<
            dyn Fn(String, Vec<String>, Vec<String>) -> Box<dyn Node>
                + Send
                + Sync
                + 'static,
        >,
...
impl NodeRegistry {
    ...
    pub fn get(
        ...
        &Box<
            dyn Fn(String, Vec<String>, Vec<String>) -> Box<dyn Node> + Send + Sync + 'static,
        >,
    > {
...
pub fn blueprint_to_graph(
    ...
    for (node_id, info) in &bp.nodes {
        ...
        let outputs = output_ports
            ...
            .cloned()
            .unwrap_or_default();
        let node: Box<dyn Node> = match registry.and_then(|reg| {
            info.kind.as_deref().and_then(|k| reg.get(k))
        }) {
...
#[cfg(test)]
mod tests {
    use super::{blueprint_to_graph, NodeRegistry};
    ...
    fn blueprint_to_graph_builds_graph_with_placeholders() {
        ...
        assert!(mmd.contains("streamweave"), "export should contain streamweave block");
    ...
    fn blueprint_to_graph_with_registry_uses_constructor_for_kind() {
        ...
        info.kind = Some("Custom".to_string());
