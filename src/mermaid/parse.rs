...
pub fn mmd_to_blueprint(mmd: &str) -> Result<GraphBlueprint, ParseError> {
    use mermaid_rs_renderer::{parse_mermaid, DiagramKind};
    ...
    for edge in &graph.edges {
        ...
        loop {
            let trimmed = label_str
                ...
                .or_else(|| label_str.strip_prefix('"').and_then(|s| s.strip_suffix('"')));
            match trimmed {
    ...
    for line in mmd.lines() {
        ...
        if let Some(rest) = payload.strip_prefix("input ") {
            if let Some((ext, in_part)) = rest.split_once(" -> ") {
                if let Some((node, port)) = in_part.split_once('.') {
            ...
            if let Some((ext, out_part)) = rest.split_once(" <- ") {
                if let Some((node, port)) = out_part.split_once('.') {
            ...
            if let Some((a, b)) = rest.split_once('/') {
                if let (Ok(id), Ok(total)) = (a.trim().parse::<u32>(), b.trim().parse::<u32>()) {
            ...
            if let Some((internal, external)) = rest.split_once('=') {
...
"#;
        ...
        assert!(bp.nodes.len() >= 2, "expected at least 2 nodes, got {}", bp.nodes.len());
        ...
        assert!(with_label.is_some(), "expected an edge with label out->in, got edges: {:?}", bp.edges);
    }
    ...
    fn parse_fixture_from_export_roundtrip() {
        ...
        assert_eq!(roundtrip.inputs.len(), original.inputs.len(), "I/O inputs preserved");
        assert_eq!(roundtrip.outputs.len(), original.outputs.len(), "I/O outputs preserved");
        ...
        assert!(has_convention_edge || !roundtrip.edges.is_empty(), "structure preserved");
    }
...
"#;
    ...
    fn roundtrip_export_parse_structural_equality() {
        ...
        for id in bp1.nodes.keys() {
            assert!(bp2.nodes.contains_key(id), "node {id} preserved in roundtrip");
        }
        ...
        let input_set = |bp: &GraphBlueprint| {
            bp.inputs
                .iter()
                .map(|b| (b.external_name.clone(), b.node_id.clone(), b.port_name.clone()))
                .collect::<BTreeSet<_>>()
        };
    ...
    fn roundtrip_with_sidecar_preserves_name_and_metadata() {
        ...
        assert_eq!(input_set(&bp1), input_set(&bp2), "inputs preserved");
        assert_eq!(bp1.execution_mode, bp2.execution_mode, "execution_mode preserved");
    }
}
