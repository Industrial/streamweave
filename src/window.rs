...
impl BoundedOutOfOrdernessGenerator {
  ...
  fn calculate_watermark(&self) -> Watermark {
    match self.max_timestamp {
      Some(max_ts) => {
        let out_of_orderness_chrono =
          ChronoDuration::from_std(self.max_out_of_orderness).unwrap_or(ChronoDuration::zero());
        Watermark::new(max_ts - out_of_orderness_chrono)
      }
      None => Watermark::min(),
    }
  }
