...
//! - **Input**: `"in"` - Receives data items to repeat
//! - **Input**: `"count"` - Receives count value (numeric: how many times to repeat each item)
//! - **Output**: `"out"` - Sends each item repeated N times
//! - **Output**: `"error"` - Sends errors that occur during processing
//!
//! ## Behavior
//!
//! The node repeats each item from the input stream N times, where N is received on the "count" port.
//! It supports:
//! - Repeating items based on a numeric count (usize, i32, i64, u32, u64)
//! - Error handling: Invalid count values (negative, wrong type) result in errors sent to the error port
//! - If count is not set yet, items are buffered until count is received
...
enum InputPort {
  /// Input data port.
  In,
  /// Count port specifying number of repetitions.
  Count,
}
...
impl RepeatNode {
  /// Creates a new RepeatNode with the given name.
  ///
  /// # Arguments
  ///
  ...
  /// # Returns
  ///
  /// A new `RepeatNode` instance.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  /// use streamweave::nodes::advanced::RepeatNode;
  ///
  /// let node = RepeatNode::new("repeat".to_string());
          ...
          match port {
            InputPort::Count => {
              ...
              match get_usize(&item) {
                Ok(c) => {
                  count = Some(c);
                  // Process any buffered items now that we have a count
                  for buffered_item in buffered_items.drain(..) {
                    if let Some(repeat_count) = count {
                      for _ in 0..repeat_count {
                        let _ = out_tx_clone.send(buffered_item.clone()).await;
                      }
                    }
                  }
                }
            ...
            InputPort::In => {
              // Repeat item if count is set, otherwise buffer it
              if let Some(repeat_count) = count {
                for _ in 0..repeat_count {
                  let _ = out_tx_clone.send(item.clone()).await;
                }
              } else {
                // Buffer item until count is received
