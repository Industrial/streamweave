...
#[derive(Clone)]
pub(crate) struct EventTimeExtractorConfigInner(Arc<dyn EventTimeExtractor>);
...
impl EventTimeExtractor for HasEventTimeExtractor {
    async fn event_time_ms(&self, value: Arc<dyn Any + Send + Sync>) -> Option<u64> {
        ...
        if let Ok(arc_map) = value.clone().downcast::<HashMap<String, Arc<dyn Any + Send + Sync>>>()
        {
...
pub fn event_time_from_map() -> EventTimeExtractorConfig {
    Arc::new(EventTimeExtractorConfigInner(Arc::new(HasEventTimeExtractor)))
}
...
{
    Arc::new(EventTimeExtractorConfigInner(Arc::new(ClosureExtractor { f })))
}
    ...
    > {
        ...
        Box::pin(async move {
            let (out_rx, error_rx) = process_configurable_node(
                inputs.remove("configuration").ok_or("Missing configuration")?,
