...
#[tokio::test]
async fn test_last_node_creation() {
  let node = LastNode::new("test_last".to_string());
  assert_eq!(node.name(), "test_last");
...
#[tokio::test]
async fn test_last_node_emits_last_item() {
  let node = LastNode::new("test_last".to_string());
  ...
  // Should have received only the last item
...
#[tokio::test]
async fn test_last_node_empty_stream() {
  let node = LastNode::new("test_last".to_string());
...
#[tokio::test]
async fn test_last_node_single_item() {
  let node = LastNode::new("test_last".to_string());
...
#[tokio::test]
async fn test_last_node_string_values() {
  let node = LastNode::new("test_last".to_string());
  ...
  // Send string values: "first", "second", "third", "last"
  let test_values = vec!["first", "second", "third", "last"];
  for value in test_values {
    in_tx
  ...
  // Should have received only the last string
  assert_eq!(results.len(), 1);
