...
use crate::time::{
  DifferentialElement, DifferentialStreamMessage, LogicalTime, StreamMessage, Timestamped,
};
    ...
    Box::pin(async move {
      ...
      tokio::spawn(async move {
        ...
        while let Some(item) = in_stream.next().await {
          ...
          if is_watermark {
            let wm =
              Arc::new(DifferentialStreamMessage::<Arc<dyn Any + Send + Sync>>::Watermark(time))
