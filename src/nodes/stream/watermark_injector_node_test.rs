...
async fn test_watermark_injector_emits_watermark_on_eos() {
    ...
    map.insert("event_timestamp".to_string(), Arc::new(100i64) as Arc<dyn Any + Send + Sync>);
    map.insert("value".to_string(), Arc::new(1i32) as Arc<dyn Any + Send + Sync>);
    in_tx.send(Arc::new(map)).await.unwrap();

    let mut map2 = HashMap::new();
    map2.insert("event_timestamp".to_string(), Arc::new(200i64) as Arc<dyn Any + Send + Sync>);
    map2.insert("value".to_string(), Arc::new(2i32) as Arc<dyn Any + Send + Sync>);
