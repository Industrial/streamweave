...
fn event_time_from_payload(item: &Arc<dyn Any + Send + Sync>) -> Option<u64> {
    let arc_map = item.clone().downcast::<HashMap<String, Arc<dyn Any + Send + Sync>>>().ok()?;
    let map = arc_map.as_ref();
    let ts = map.get("event_timestamp").or_else(|| map.get("timestamp"))?;
                ...
                loop {
                    match in_stream.next().await {
                        None => {
                            if max_time > 0 {
                                let _ = out_tx
                                    .send(Arc::new(StreamMessage::<Arc<dyn Any + Send + Sync>>::Watermark(
                            ...
                            if let Some(msg) = try_stream_message(item.clone()) {
                                match msg {
                                    ...
                                    StreamMessage::Watermark(w) => {
                                        ...
                                        max_time = max_time.max(t);
                                        let out =
                                            Arc::new(StreamMessage::<Arc<dyn Any + Send + Sync>>::Watermark(w))
                                                as Arc<dyn Any + Send + Sync>;
                                        let _ = out_tx.send(out).await;
                                    }
                                }
                                ...
                                max_time = max_time.max(t);
                                let out =
                                    Arc::new(StreamMessage::Data(ts)) as Arc<dyn Any + Send + Sync>;
                                let _ = out_tx.send(out).await;
                            } else {
