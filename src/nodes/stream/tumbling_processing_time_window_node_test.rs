...
async fn test_tumbling_window_creation() {
  let node =
    TumblingProcessingTimeWindowNode::new("tumbling".to_string(), Duration::from_millis(100));
...
async fn test_tumbling_window_emits_on_stream_end() {
  let node =
    TumblingProcessingTimeWindowNode::new("tumbling".to_string(), Duration::from_millis(500));
