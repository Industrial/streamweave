...
#[tokio::test]
async fn test_distinct_node_creation() {
  let node = DistinctNode::new("test_distinct".to_string());
  assert_eq!(node.name(), "test_distinct");
...
async fn test_distinct_all_unique() {
  ...
  // Should have received all 5 unique items
...
async fn test_distinct_with_duplicates() {
  ...
  // Should have received only the unique items: 1, 2, 3, 4
...
#[tokio::test]
async fn test_distinct_single_item() {
  let node = DistinctNode::new("test_distinct".to_string());
