...
impl Node for DifferentialJoinNode {
  ...
  > {
    ...
    Box::pin(async move {
      ...
      let merged: Pin<Box<dyn futures::Stream<Item = MergedItem> + Send>> =
            ...
            Port::Config => {
              if let Ok(arc) = item.clone().downcast::<Arc<JoinConfig>>() {
                ...
                *config_state.lock().await = Some(Arc::clone(&*arc));
                ...
                let _ = error_tx
                  .send(Arc::new(
                    "Invalid configuration type - expected Arc<JoinConfig>".to_string(),
                    ...
                    for (rv, _rt, rd) in right_buf.get(&key).cloned().unwrap_or_default() {
                        ...
                        .await
                      {
                        Ok(joined) => {
                          ...
                          if out_diff != 0 {
                            let out_elem = DifferentialElement::new(joined, time, out_diff);
                            let _ = out_tx
