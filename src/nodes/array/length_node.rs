//! # Array Length Node
//!
//! A transform node that returns the length of an array.
//!
//! ## Ports
//!
//! - **Input**: `"configuration"` - Receives configuration (currently unused, for consistency)
//! - **Input**: `"in"` - Receives array value (Vec<Arc<dyn Any + Send + Sync>>)
//! - **Output**: `"out"` - Sends the array length (usize)
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., type mismatch)
//!
//! ## Behavior
//!
//! The node returns the length of an array and returns the result. It supports:
...
use crate::nodes::array::common::array_length;
...
/// A node that returns the length of an array.
///
/// The node receives an array value on the "in" port and outputs
/// the array length (usize) to the "out" port.
pub struct ArrayLengthNode {
  pub(crate) base: BaseNode,
}

impl ArrayLengthNode {
  /// Creates a new ArrayLengthNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  ...
  ///
  /// let node = ArrayLengthNode::new("length".to_string());
