//! Tests for ArrayLengthNode

use crate::node::{InputStreams, Node};
use crate::nodes::array::ArrayLengthNode;
use std::any::Any;
...
#[tokio::test]
async fn test_array_length_node_creation() {
  let node = ArrayLengthNode::new("test_length".to_string());
...
async fn test_array_length_single_element() {
  ...
  // Send value: [42] → 1
  let vec: Vec<Arc<dyn Any + Send + Sync>> = vec![Arc::new(42i32) as Arc<dyn Any + Send + Sync>];
  let _ = in_tx
    .send(Arc::new(vec) as Arc<dyn Any + Send + Sync>)
    .await;
  ...
  assert_eq!(results.len(), 1);
  assert_eq!(*results[0], 1usize);
}

#[tokio::test]
async fn test_array_length_mixed_types() {
  let node = ArrayLengthNode::new("test_length".to_string());
  ...
  // Send value: [1, "hello", 3.14] → 3
  let vec: Vec<Arc<dyn Any + Send + Sync>> = vec![
    Arc::new(1i32) as Arc<dyn Any + Send + Sync>,
    Arc::new("hello".to_string()) as Arc<dyn Any + Send + Sync>,
    Arc::new(2.5f64) as Arc<dyn Any + Send + Sync>,
  ];
  let _ = in_tx
    .send(Arc::new(vec) as Arc<dyn Any + Send + Sync>)
    .await;
