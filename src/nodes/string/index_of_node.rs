...
//!
//! A transform node that finds the index of a substring within a string.
//!
//! ## Ports
//!
...
//! - **Output**: `"out"` - Sends the index as i64 (-1 if not found)
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., type mismatch)
//!
//! ## Behavior
//!
//! The node finds the first occurrence of a substring within a string and returns its index.
...
use crate::nodes::string::common::string_index_of;
...
impl StringIndexOfNode {
  /// Creates a new StringIndexOfNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  ...
  ///
  /// let node = StringIndexOfNode::new("my_index_of".to_string());
  /// ```
...
#[async_trait]
impl Node for StringIndexOfNode {
  fn name(&self) -> &str {
    self.base.name()
  }
      ...
      tokio::spawn(async move {
        ...
        while let Some((port, item)) = merged.next().await {
          ...
          if let (Some(v), Some(substring)) = (in_buffer.as_ref(), substring_buffer.as_ref()) {
            match string_index_of(v, substring) {
              Ok(result) => {
