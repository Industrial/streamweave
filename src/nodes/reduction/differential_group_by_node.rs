...
impl Node for DifferentialGroupByNode {
    ...
    > {
        Box::pin(async move {
            ...
            let key_fn_stream = inputs.remove("key_function").ok_or("Missing 'key_function' input")?;
                    ...
                    match port {
                        ...
                        InputPort::KeyFunction => {
                            if let Ok(wrapper) = Arc::downcast::<GroupByConfigWrapper>(item.clone()) {
                                        ...
                                        match msg {
                                            DifferentialStreamMessage::Data(elem) => {
                                                if let Some(kf) = &key_fn {
                                                    if let Ok(key) = kf.extract_key(elem.payload().clone()).await {
                                                        let time = elem.time();
                                                        let diff = elem.diff();
                                ...
                                let _ = error_tx
                                    .send(Arc::new("Invalid key_function (expected GroupByConfigWrapper)".to_string())
                                        as Arc<dyn Any + Send + Sync>)
                                    .await;
                            }
                        }
