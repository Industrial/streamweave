//! # Divide Node
//!
//! A transform node that performs division on two numeric inputs.
//!
//! ## Ports
//!
//! - **Input**: `"configuration"` - Receives configuration (currently unused, for consistency)
//! - **Input**: `"in1"` - Receives first numeric value (dividend)
//! - **Input**: `"in2"` - Receives second numeric value (divisor)
//! - **Output**: `"out"` - Sends the result of (in1 / in2)
//! - **Output**: `"error"` - Sends errors that occur during processing (e.g., division by zero, type mismatch)
//!
//! ## Behavior
//!
//! The node performs division on two numeric values. It supports:
...
//! - Division by zero detection: all operations check for division by zero and emit errors

use crate::node::{InputStreams, Node, NodeExecutionError, OutputStreams};
use crate::nodes::arithmetic::common::divide_values;
...
enum InputPort {
  /// First input operand (dividend).
  In1,
  /// Second input operand (divisor).
  In2,
}

/// A node that performs division on two numeric inputs.
///
/// The node receives two numeric values on "in1" (dividend) and "in2" (divisor) ports
/// and outputs the result of the division to the "out" port.
...
impl DivideNode {
  /// Creates a new DivideNode with the given name.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  ...
  ///
  /// let node = DivideNode::new("divide".to_string());
