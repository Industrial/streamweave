    ...
    Box::pin(async move {
      ...
      tokio::spawn(async move {
        while let Some(item) = in_stream.next().await {
          if let Ok(n) = item.downcast::<i32>() {
            let out = *n / 2 + 1;
            let _ = tx
              .send(Arc::new(out) as Arc<dyn std::any::Any + Send + Sync>)
              .await;
          }
        }
      });
      ...
      out.insert(
        ...
        Box::pin(ReceiverStream::new(rx))
          as Pin<Box<dyn tokio_stream::Stream<Item = Arc<dyn std::any::Any + Send + Sync>> + Send>>,
      );
      Ok(out)
    })
  }
...
async fn test_bounded_iteration_single_round() {
  ...
  seed_tx
    .send(Arc::new(42i32) as Arc<dyn std::any::Any + Send + Sync>)
    .await
    .unwrap();
  drop(seed_tx);
...
async fn test_bounded_iteration_two_rounds() {
  ...
  seed_tx
    .send(Arc::new(7i32) as Arc<dyn std::any::Any + Send + Sync>)
    .await
    .unwrap();
  drop(seed_tx);
...
async fn test_bounded_iteration_fixed_point() {
  ...
  seed_tx
    .send(Arc::new(10i32) as Arc<dyn std::any::Any + Send + Sync>)
    .await
    .unwrap();
  drop(seed_tx);
