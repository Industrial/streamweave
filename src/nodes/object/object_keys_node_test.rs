//! Tests for ObjectKeysNode

use crate::node::{InputStreams, Node};
use crate::nodes::object::object_keys_node::ObjectKeysNode;
use std::any::Any;
...
#[tokio::test]
async fn test_object_keys_node_creation() {
  let node = ObjectKeysNode::new("test_keys".to_string());
  assert_eq!(node.name(), "test_keys");
...
async fn test_object_keys_basic() {
  ...
  // Send values: {"name": "John", "age": 30} → ["name", "age"]
...
#[tokio::test]
async fn test_object_keys_non_object_input() {
  let node = ObjectKeysNode::new("test_keys".to_string());
